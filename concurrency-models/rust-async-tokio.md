...
2. **Reactor（基于 epoll/kqueue/IOCP）**：监听 IO 事件。当一个 Future 因为等 IO 返回 `Pending` 时，Tokio 把它「挂起」，去 poll 别的 task。这里的 **Reactor** 和 [3.6 Java 网络 IO 模型](../part3-java-deep/06-网络IO模型.md) 里 Netty/Node 用的是**同一个设计模式**——一个线程用 epoll 盯住海量连接、就绪了再分派，只是 Rust 把它和「无栈状态机 + Waker」缝合在了一起。
...
| 底层 IO | epoll/kqueue (Reactor) | 同 [Node libuv](./nodejs-eventloop.md)、[3.6 Java NIO](../part3-java-deep/06-网络IO模型.md) |
| 性能/内存 | 极致 | 最强 |

...
- [7.12 案例研究：大模型数据工程与评测岗 JD 拆解——招聘方尽调 / persona 数据构建 / CharacterEval / LLM-as-a-Judge 校准 / 医疗合规 / 20 道预测题 / 16 个反问](./part7-ai-engineering/12-案例-大模型数据评测岗JD拆解.md)
...
**路径 B · 问题驱动（带着具体问题查阅）**：直接从第四章的对比案例切入，遇到 Java 基准不熟就回溯第三章，遇到前端概念不懂就回溯第二章，遇到思维卡壳就回第一章。AI 相关问题直接查第七章。Notebook 开发面试或平台架构设计直接查第八章。自动驾驶数据平台岗位查第九章，KMS/国密/量子网络岗位查第十章，高并发中台/标签策略平台岗位查第十一章，大模型数据工程/模型评测岗位查 [7.12](./part7-ai-engineering/12-案例-大模型数据评测岗JD拆解.md)（这一节还附带一套「如何对早期创业公司做尽调」的通用方法论）。

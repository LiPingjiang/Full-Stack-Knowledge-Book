...
这本书的前七章是一条精心设计的学习路径，环环相扣。第八章是全栈知识的综合应用案例——以 Notebook 平台为切入点，串联前端、后端、数据、AI 四大领域。第九至十四章是岗位案例研究——拿真实或可重建的岗位场景拆解，验证知识体系的覆盖度，并把通用能力迁移到自动驾驶、密码安全、高并发中台、现代数据工程、国际化外卖数据开发，以及大模型训练数据与 Agent 可观测等垂直领域；第十五章是这个系列的最新一环，也是最「总装型」的一环——垂直领域（生物医药）数据底座 + 知识工程 + AI 平台 + ToB 交付的负责人岗：
...
flowchart LR
    ...
    C8 --> C9["第九章<br/>自动驾驶数据平台案例<br/>(知识验证与补缺)"]
    C9 --> C10["第十章<br/>密码安全与量子网络案例<br/>(领域能力深化)"]
    C10 --> C11["第十一章<br/>标签策略中台案例<br/>(通用能力验证)"]
    C11 --> C12["第十二章<br/>iQAir·Snowflake 数据工程案例<br/>(现代数仓组织能力)"]
    C12 --> C13["第十三章<br/>国际化外卖数仓案例<br/>(多时区+AI基建复合岗位)"]
    C13 --> C14["第十四章<br/>阶跃 StepFun AI 数据案例<br/>(训练数据+评测+Agent可观测)"]
    C14 --> C15["第十五章<br/>生物医药数据与AI体系案例<br/>(领域数据+知识工程+ToB交付)"]
...
- [附录 A10：认证、授权与凭证安全——SSO / OAuth 2.0 / OIDC / JWT / BFF / Token Exchange / CIBA / mTLS / STS](./part3-java-deep/A10-认证授权与凭证安全.md)
...
- [6.18 TiDB 与 HTAP 数据库：TiDB Server / PD / TiKV / TiFlash / Raft 多副本 / HTAP 选型](./part6-bigdata/18-TiDB与HTAP数据库.md)
- [6.19 元数据管理与智能数据治理：数据血缘 / 图数据库 / AI 辅助元数据生成 / 智能质量巡检](./part6-bigdata/19-元数据管理与智能数据治理.md)
...
- [7.12 案例研究：点富科技「大模型数据工程与评测岗」深度分析——公司背景核验 / 专家数字分身技术拆解 / persona 数据构建 / CharacterEval / LLM-as-a-Judge 校准 / 医疗合规 / **数据体系 0→1 建设规划** / **赛道标尺与研发模式** / **产品定位诊断（市场漏斗数据·B端破局）** / **监管边界专题（21号令三要件·中美欧英对比·合规资质清单）** / **实证核查（现有产品落在监管哪一侧）** / **竞品全景（中外 30+ 家分层对比·护城河判断）** / **技术路线终局（SOTA vs 垂类微调的分层结论）** / **市场真实性验证（预测 vs 已实现收入）** / **突破点分析（通用 Agent 的结构性硬伤与四条已验证插入点）** / **数据建设独特性（FHIR/OMOP 医疗标准 + LLM Trace 资产 + 合规架构内建）** / **PsyDT 论文深读与评测方法论工具箱（Rubric / Cohen's Kappa / CCE / 偏见检测）** / 28 道预测题 / 23 个反问](./part7-ai-engineering/12-案例-点富科技大模型数据评测岗.md)
...
### 第十章 · 案例研究：密码基础设施与量子网络密钥管理 〔领域深化〕

以 KMS、CMSP、KAAS 与量子网络岗位为案例，覆盖密钥管理、HSM/PKI、国密、密评和量子密钥调度。Java、分布式、高可用、多租户、SDK 与服务网格等通用能力在此章节中被映射为安全密码工程实践。

- [本章导读](./part10-case-crypto-kms-quantum/README.md)
- [10.1 岗位拆解与知识映射](./part10-case-crypto-kms-quantum/01-岗位拆解与知识映射.md)
- [10.2 密码基础设施全景：KMS、PKI、HSM 与密钥生命周期](./part10-case-crypto-kms-quantum/02-密码基础设施全景.md)
- [10.3 通用知识应用：高可用、多租户、SDK 与服务治理](./part10-case-crypto-kms-quantum/03-通用知识应用.md)
- [10.4 领域知识：国密、密评与量子密钥网络](./part10-case-crypto-kms-quantum/04-领域知识.md)
- [10.5 面试准备路径与高频问题](./part10-case-crypto-kms-quantum/05-面试准备.md)

### 第十一章 · 案例研究：标签与策略平台高并发中台架构 〔通用能力验证〕

以出行平台（滴滴）标签和策略平台后端研发岗为案例，验证第三章 Java 深度知识体系对一线互联网高并发中台岗位的覆盖度。这份 JD 几乎不涉及冷门领域知识，考察的是并发、JVM、Redis、消息队列、Spring、分布式锁这些通用硬功夫，全书覆盖度在三个案例中最高，真正的缺口集中在 Harness Engineering、Agent 工程化和标签系统规则引擎设计上。

- [本章导读](./part11-case-tag-strategy-platform/README.md)
- [11.1 岗位拆解与知识映射](./part11-case-tag-strategy-platform/01-岗位拆解与知识映射.md)
- [11.2 高并发中台架构全景：从标签存储到策略决策](./part11-case-tag-strategy-platform/02-高并发中台架构全景.md)
- [11.3 通用知识缺口：Harness Engineering、Agent 工程化、RPC 框架深度](./part11-case-tag-strategy-platform/03-通用知识缺口.md)
- [11.4 领域知识：标签系统与规则引擎设计](./part11-case-tag-strategy-platform/04-领域知识.md)
- [11.5 面试准备路径与高频问题](./part11-case-tag-strategy-platform/05-面试准备.md)

---

### 第十二章 · 案例研究：iQAir 空气质量数据平台与 Snowflake 数仓建设 〔现代数据工程组织能力〕

以 iQAir 这类全球空气质量平台为案例，分析一个“从零散数据走向现代数仓”的数据工程岗位：如何从公开产品页反推数据域，如何在 Snowflake 上组织 Raw / Bronze / Silver / Gold，哪些内容应该沉淀为第六章的通用能力，哪些内容属于空气质量场景特有的领域知识。

- [本章导读](./part12-case-iqair-snowflake/README.md)
- [12.1 岗位拆解与知识映射](./part12-case-iqair-snowflake/01-岗位拆解与知识映射.md)
- [12.2 空气质量数据平台与数仓架构全景](./part12-case-iqair-snowflake/02-空气质量数据平台与数仓架构全景.md)
- [12.3 通用知识补缺与章节回链](./part12-case-iqair-snowflake/03-通用知识补缺与章节回链.md)
- [12.4 领域知识：空气质量、时序、地理空间与多源对账](./part12-case-iqair-snowflake/04-领域知识：空气质量、时序、地理空间与多源对账.md)
- [12.5 面试准备路径与高频问题](./part12-case-iqair-snowflake/05-面试准备.md)

---

### 第十三章 · 案例研究：国际化外卖数据开发——多时区、多语言、多币种下的数仓建设 〔国际化+AI 基建复合能力〕

以某出行平台国际化外卖业务线数据开发岗位为案例，分析一个"流量/营销域数仓 + 数据中台基建 + 数据治理 + AI 知识库"的复合型岗位：为什么国际化外卖的数仓建设比国内外卖多了三个维度（时区、语言、币种），为什么它既要求维度建模硬功夫又要求 RAG 知识库能力，以及哪些能力属于通用数仓工程，哪些能力是国际化场景特有的。

- [本章导读](./part13-case-intl-food-delivery-data/README.md)
- [13.1 岗位拆解与知识映射](./part13-case-intl-food-delivery-data/01-岗位拆解与知识映射.md)
- [13.2 国际外卖数仓架构全景](./part13-case-intl-food-delivery-data/02-国际外卖数仓架构全景.md)
- [13.3 通用知识补缺与章节回链](./part13-case-intl-food-delivery-data/03-通用知识补缺与章节回链.md)
- [13.4 领域知识：多时区、多语言、多币种与营销归因](./part13-case-intl-food-delivery-data/04-领域知识：多时区多语言多币种与营销归因.md)
- [13.5 面试准备路径与高频问题](./part13-case-intl-food-delivery-data/05-面试准备.md)

---

### 第十四章 · 案例研究：阶跃 StepFun AI 数据处理、分析与 Agent 可观测 〔训练数据+评测+生产闭环〕

以用户提供的“AI 数据处理分析工程师”岗位描述为起点，结合阶跃星辰公开招聘、产品信息与 StepTrace 技术分享，重建一个“训练数据工程 + 数据平台 + Agent 评测与可观测”的复合型岗位案例。该章节明确区分公开可核验事实、用户提供的岗位描述和架构推演，重点训练如何把预训练/SFT/偏好/评测数据与生产 Agent Trace 组织为可追溯、可验证的闭环。

- [本章导读](./part14-case-stepfun-ai-data/README.md)
- [14.0 一页速览：岗位本质、证据边界与准备重点](./part14-case-stepfun-ai-data/00-一页速览.md)
- [14.1 岗位拆解与知识映射](./part14-case-stepfun-ai-data/01-岗位拆解与知识映射.md)
- [14.2 统一数据闭环架构：训练数据、评测与 Agent Trace](./part14-case-stepfun-ai-data/02-统一数据闭环架构.md)
- [14.3 通用知识补缺与学习路径](./part14-case-stepfun-ai-data/03-通用知识补缺与学习路径.md)
- [14.4 领域知识：LLM 数据质量、评测与可观测](./part14-case-stepfun-ai-data/04-领域知识：LLM数据质量评测与可观测.md)
- [14.5 面试准备：自我评价、实战题与反问清单](./part14-case-stepfun-ai-data/05-面试准备.md)

---

### 第十五章 · 案例研究：生物医药数据与 AI 体系负责人（AI 制药初创） 〔领域数据+知识工程+ToB交付〕

以一份真实的「数据与AI负责人（生物医药方向）」岗位描述为起点（招聘方做匿名化处理，艺名「青澜生物」），结合业界公开案例调研（Genentech、AstraZeneca、Sanofi、Servier、复星医药、晶泰科技、医渡科技），重建一个「生物医药数据底座 + 知识图谱/Ontology + RAG/Agent + ToB 交付」的创业公司核心岗案例。重点包括：四层架构蓝图与初创裁剪版（砍什么、留什么、为什么）、核心实体统一公式（靶点/疾病/分子/专利/临床）、CDISC/FHIR/OMOP 标准分工、GxP 边界、中文术语映射层，以及初创版 90 天计划与尽调清单。这是全书第一次把第六、七章的知识在「垂直领域 + 小团队 + 负责人亲自产出」的约束下总装。

- [本章导读](./part15-case-biopharma-data-ai/README.md)
- [15.0 一页速览：岗位本质、初创特殊性与准备重点](./part15-case-biopharma-data-ai/00-一页速览.md)
- [15.1 岗位拆解与知识映射：JD 反模式警告与能力模型](./part15-case-biopharma-data-ai/01-岗位拆解与知识映射.md)
- [15.2 生物医药数据与 AI 基础设施架构全景：蓝图、业界参照与初创裁剪](./part15-case-biopharma-data-ai/02-生物医药数据与AI基础设施架构全景.md)
- [15.3 通用知识补缺与章节回链](./part15-case-biopharma-data-ai/03-通用知识补缺与章节回链.md)
- [15.4 领域知识：生物医药研发数据实体、标准与 Ontology](./part15-case-biopharma-data-ai/04-领域知识：生物医药研发数据实体标准与Ontology.md)
- [15.5 面试准备：初创 90 天计划、实战题、反问与尽调清单](./part15-case-biopharma-data-ai/05-面试准备.md)
...
**路径 A · 按部就班（推荐首次阅读）**：第一章 → 第二章 → 第三章 → 第四章（遇到并发模型链接就跳进引用库）→ 第五章 → 第六章 → 第七章 → 第八章 → 第九章 → 第十章 → 第十一章 → 第十二章 → 第十三章 → 第十四章 → 第十五章。前三章是地基，第四章是融会贯通，第五章是加速器，第六章是数据视野，第七章是 AI 工程认知，第八章是全栈综合应用案例，第九至十四章是知识体系在真实岗位中的验证、补缺与深化，第十五章把这套能力总装到「垂直领域 AI 数据负责人」的创业场景。

**路径 B · 问题驱动（带着具体问题查阅）**：直接从第四章的对比案例切入，遇到 Java 基准不熟就回溯第三章，遇到前端概念不懂就回溯第二章，遇到思维卡壳就回第一章。AI 相关问题直接查第七章。Notebook 开发面试或平台架构设计直接查第八章。自动驾驶数据平台岗位查第九章，KMS/国密/量子网络岗位查第十章，高并发中台/标签策略平台岗位查第十一章，现代数仓 / Snowflake / 数据产品化的数据工程岗位查第十二章，国际化外卖 / 多时区数仓 / 数据+AI 复合岗位查第十三章；涉及阶跃这类大模型公司中的训练数据、模型评测、实时数据平台或 Agent Trace 可观测岗位，查第十四章；生物医药 / AI 制药 / 垂直领域数据与知识体系负责人岗位（以及医疗、法律、材料等同类垂直 AI 数据岗），查第十五章。大模型数据工程/模型评测岗位（以点富科技为具体案例）可查 [7.12](./part7-ai-engineering/12-案例-点富科技大模型数据评测岗.md)，其中还附带一套“如何对早期创业公司做尽调”的通用方法论。

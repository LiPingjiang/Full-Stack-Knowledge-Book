...
```
传统: for each row → evaluate expression → output
向量化: load 8192 values into CPU cache → SIMD batch compute → output batch
```

**优势**：减少虚函数调用、利用 CPU SIMD 指令（SSE4.2/AVX2）、缓存命中率高。

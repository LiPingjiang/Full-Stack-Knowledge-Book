...
最优解法：开启 MiniBatch + LocalGlobal（详见 6.1 节），将多次 State 访问合并为一次，吞吐量提升数倍。
...
RocksDB 参数调优详见 5.3 节（包括多磁盘目录、SSD 优化、Block Cache、MemTable 参数等）。
...
- **两阶段聚合（有窗口场景）**：先局部预聚合（如加随机前缀 `keyBy(randomPrefix + key)`），再全局聚合。Flink SQL 会自动做 `Local-Global` 聚合优化（详见 6.1.2）。注意：有窗口时，第二阶段聚合必须携带 `windowEnd` 作为分组依据，避免不同窗口的结果被混在一起。

...
```mermaid
timeline
    title 用户 A 的数据流（Session Window, gap=30min）
    section 会话 1（10:00 ~ 11:15）
        10:00 : 访问
        10:05 : 访问（间隔5min < 30min）
        10:20 : 访问（间隔15min < 30min）
        10:45 : 访问（间隔25min < 30min）
        11:15 : 10:45+30min 无新数据 → 会话关闭
    section 间隔 45min > 30min → 拆分会话
        （无活动）: ──
    section 会话 2（11:30 ~ 12:05）
        11:30 : 访问
        11:35 : 访问（间隔5min < 30min）
        12:05 : 11:35+30min 无新数据 → 会话关闭
```

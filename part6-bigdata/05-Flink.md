...
#### 1. 选择 EmbeddedRocksDBStateBackend（大 State 生产首选）

如果 State 能稳定放进 JVM 堆并且你更看重吞吐，`HashMapStateBackend` 也可以用于生产；但对几十 GB 乃至更大的 State、长窗口、以及需要增量 Checkpoint 的场景，`EmbeddedRocksDBStateBackend` 通常是更稳妥的选择。它把 State 放在本地磁盘（RocksDB 文件）中，不受 JVM 堆大小直接限制。

```java
// 配置 EmbeddedRocksDBStateBackend，Checkpoint 保存到 HDFS
...
增量 Checkpoint 只上传新增 SST 文件，大幅减少上传数据量。但增量链过长会导致恢复变慢。生产上通常会显式开启 `execution.checkpointing.incremental: true`，而不是依赖默认值；Flink 会自动管理增量链，并在需要时形成新的全量基线。

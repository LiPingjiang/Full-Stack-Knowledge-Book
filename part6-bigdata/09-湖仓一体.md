...
> **注意**：统一访问层中的引擎角色并不相同。Spark SQL / Flink / Trino / Presto 是**纯计算引擎**（无自有存储），而 Doris 是**存储+计算一体**的引擎。Doris 查湖表时通过 Multi-Catalog 机制退化为纯计算引擎（不使用自己的本地存储），直接从 S3/HDFS 拉取 Parquet 文件到 BE 内存中用[向量化引擎](./06-Doris.md#41-列式存储--向量化执行)执行计算。详见下文"统一访问层引擎对比"。

...
- **多索引** — [Bloom Filter](../part3-java-deep/A1-核心数据结构原理.md#三布隆过滤器bloom-filter用-1-的误判换-99-的内存节省) Index / HBase Index / Bucket Index 加速 Upsert
...
前文重点讲的是以 Iceberg / Hudi / Paimon 为代表的开放湖仓。现实工作里，很多团队却会说：“我们不用自己搭 Iceberg，我们直接用 Snowflake / BigQuery 也在做湖仓。”这句话并不完全错误。

### 9.1 为什么很多团队会把托管云数仓也叫“湖仓”

因为从用户视角看，它同样满足了现代湖仓最关心的几个目标：
- **存算分离**：存储和计算资源独立扩缩；
- **原始数据与分析数据共存**：既能接半结构化原始数据，也能做关系型分析建模；
- **增量与批处理并存**：支持近实时摄取与批量加工；
- **共享与治理能力内建**：权限、共享、审计往往比自建湖仓更完善。

所以在业务团队嘴里，“湖仓”很多时候不是一个严格的开源技术定义，而是“既能接原始数据、又能做数仓分析、还能治理和共享的一体化数据平台”。
...
| 维度 | 托管云数仓 | 开放湖仓 |
|------|------------|----------|
| **数据控制权** | 元数据与执行引擎更强绑定到厂商平台 | 数据文件与元数据协议更开放 |
| **跨引擎互操作** | 取决于厂商支持程度 | 以开放表格式和开放 Catalog 为核心 |
| **平台运维成本** | 低，更多是配置与建模工作 | 高，需要治理对象存储、Catalog、计算引擎、维护任务 |

简单说：**托管云数仓更像“封装好的湖仓产品”，开放湖仓更像“可自主组合的湖仓底座”。**

### 9.3 什么场景更适合托管云数仓

当团队的核心目标是：
- 快速把零散数据组织成可分析、可共享、可治理的数据产品；
- 数据工程团队人数不多，不想维护一整套对象存储 + Catalog + Flink/Spark/Trino 体系；
- 更看重交付速度、权限治理和稳定性，而不是极致的开放互操作；
...
很多公司并不是二选一，而是：
- 原始大文件和开放数据资产放对象存储 / Iceberg；
- 关键分析与数据产品层跑在托管云数仓；
- 通过外部表、共享机制、开放格式把二者连起来。

所以面试里如果被问“你们到底是数仓还是湖仓”，更好的回答是先问清楚：
- 你说的是**底层技术形态**，还是**团队交付目标**？
- 你关心的是**开放互操作**，还是**快速构建数据产品**？

只有把这两个维度拆开，答案才不会流于口号。

...
**[6.18 TiDB 与 HTAP 数据库](./18-TiDB与HTAP数据库.md)** —— 分布式 NewSQL 与混合事务分析处理。HTAP 解决的核心问题（业务库与分析库割裂、CDC 同步延迟）、TiDB 架构（TiDB Server / PD / TiKV / TiFlash）、Region 分片与 Raft 多副本复制、TiFlash 以 Raft Learner 身份实现行存到列存的实时同步、TiDB vs 分库分表 vs Doris/ClickHouse 的选型对比。*面试剖析覆盖：HTAP 实现原理、TiDB 与分库分表的本质区别、强一致性保证机制、TiDB 与 OLAP 引擎的选型边界。*

**[6.19 元数据管理与智能数据治理](./19-元数据管理与智能数据治理.md)** —— 数据平台的神经系统。元数据三分类（技术/业务/管理元数据）、数据血缘的采集方式（静态 SQL 解析 vs 运行时埋点）与图数据库存储选型、元数据管理平台四层架构、AI 赋能数据治理的五个方向（字段语义描述自动生成、口径一致性推断、语义级智能质量巡检、智能 ETL 开发辅助、落地边界与人工审核原则）。*面试剖析覆盖：元数据平台架构设计、血缘采集与图存储选型、大模型在治理中的能力边界、智能巡检与传统统计检测的本质区别。*

**[6.20 AI Native 数仓与数仓智能体](./20-AI-Native数仓与数仓智能体.md)** —— 把 6.10/6.19/7.8 三篇拼成一张图。一次自然语言提问从 AgentBI 交互层 → 语义层 → 元数据与治理层的完整流转路径、三层各自的职责边界与依赖关系、为什么企业级方案不能让大模型跳过语义层直连数据库、面对"AI+Data"类 JD 该如何组织一个有结构而非罗列关键词的回答。*面试剖析覆盖：数仓智能体与 AgentBI 的关系、跳过语义层直连的风险、归因结论出错时按层排查的思路。*
...
- 如果你面试的岗位强调「数仓建设 + AI 赋能」（数仓智能体、Text2SQL、智能数据治理），除了 6.7/6.9/6.10 之外，重点看 6.19（元数据与智能治理）、6.18（TiDB/HTAP）以及 [第七章 7.8 AgentBI 智能分析](../part7-ai-engineering/08-AgentBI智能分析.md)；这三篇讲的是各自的技术细节，想直接把它们讲成一个完整故事，看 **6.20** 就够了——它是专门把这三篇拼成一张架构图、讲清楚层与层之间依赖关系的整合视角

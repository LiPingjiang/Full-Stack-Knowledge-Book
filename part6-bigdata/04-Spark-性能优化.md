...
> **SIGTERM 15 与 Dynamic Allocation 的关系**：当 Executor 空闲时间达到 `executorIdleTimeout`（默认 60s），Dynamic Allocation 会回收该 Executor——YARN 向 Container 发送 SIGTERM（信号 15），Executor 日志中出现 `RECEIVED SIGNAL 15: SIGTERM`。这是**正常的资源回收机制**，不是 OOM 也不是错误，无需处理。同样的信号也会在 YARN 资源抢占时出现（队列资源紧张时低优先级 Container 被抢占）。只有当 SIGTERM 导致 Shuffle 文件丢失（下游 Task 报 FetchFailed）时才需要介入——开启 External Shuffle Service 或 Shuffle Tracking 可以彻底解决。
...
Executor Exit Code 完整对照（YARN + JVM + Spark 三层）：
  Exit Code 143  → JVM 检测到 OOM，YARN 向 Container 发 SIGTERM(15) kill
                   本质是 Executor 堆内存不足 → 调大 executor.memory 或减小 executor.cores
  Exit Code 52   → Spark 自身 catch 到 OOM 异常，主动退出（System.exit(52)）
                   本质是堆内存不足 → 处理方式同 143
  Exit Code 56   → Executor 发送心跳到 Driver 失败，主动退出（HeartbeatReceiver 超时）
                   可能是 Driver 压力大/GC 严重/网络不稳定
                   → 增大 spark.executor.heartbeatInterval，同时排查 Driver 是否 OOM
  Exit Code 1    → JVM 非正常退出（通用错误码），需结合 stderr 日志定位具体原因
  Exit Code -104 → AM Container 被 YARN kill，通常是 Driver OOM
                   → 调大 spark.driver.memory 和 spark.yarn.driver.memoryOverhead

  区分方法：
    143/52 → Executor 堆内 OOM（先调 memory.fraction / 减 cores / 加 memory）
    YARN "exceeding physical memory limits" → 堆外 OOM（调 memoryOverhead）
    56 → 心跳失败（查 Driver 状态、网络）
```
...
> **OOM 调参策略优先级——先减 cores 还是先加 memory？** 答案是**先减 cores**。原因：减 cores 不增加资源申请量（YARN 层面 Container 内存不变），只是减少了并行 Task 数，让每个 Task 分到更多内存。例如 executor.cores 从 4 减到 2，每 Task 可用内存直接翻倍。如果减到 executor.cores=1 还 OOM，说明单 Task 数据量确实超出 Executor 内存承载能力，此时再调大 executor.memory。增加 executor.memory 意味着向 YARN 申请更大的 Container，受集群 Container 上限约束（离线集群通常 160G），且会挤占其他任务的资源配额。所以调参顺序是：**memory.fraction → shuffle.partitions → 减 executor.cores → 加 executor.memory**（成本递增）。
...
**广播变量导致 Driver OOM——比超时更严重的情况**

广播超时只是表面现象，更严重的情况是**广播表直接撑爆 Driver 堆内存**：

```
广播导致 Driver OOM 的完整链路：

  Driver 端执行 BroadcastExchange
    → collect 小表全量数据到 Driver JVM 堆内存
    → 如果表数据量 > Driver 可用堆内存 → java.lang.OutOfMemoryError: Java heap space
    → Driver 进程崩溃 → AM 检测到 Driver 退出 → Application 失败

  报错特征（区分广播 OOM 和普通 Driver OOM）：
    ① 异常栈中包含 BroadcastExchange / TorrentBroadcast
    ② 伴随 "WARN Client: Fail to get RpcResponse: Timeout" 高频出现
    ③ 最终 "ApplicationMaster for attempt appattempt_xxx timed out"
    ④ Spark UI 的 SQL DAG 中能看到 BroadcastExchange 节点

  排查步骤：
    ① 检查 autoBroadcastJoinThreshold 是否设得过大（默认 10MB，某些团队设到 100MB+）
    ② SQL 执行计划中搜索 BroadcastNestedLoopJoin
       → 如果有，说明存在非等值 JOIN（ON 条件是 or、<、> 等，或忘记写 ON 条件）
       → 非等值 JOIN 会被 Spark 强制走 BroadcastNestedLoopJoin，即使大表也会被广播
    ③ 检查是否对大表加了 broadcast hint（/*+ MAPJOIN(t) */ 或 /*+ BROADCAST(t) */）
    ④ 如果上述都排除，检查 Driver heap 分配是否偏小
       → spark.driver.memory 默认 1G，生产环境建议至少 2-4G

  修复：
    禁用自动广播：SET spark.sql.autoBroadcastJoinThreshold = -1;
    移除手动 broadcast hint
    增大 driver.memory（治标，不治本）
```
...
| Executor 堆内 OOM（exit 52） | `exit code 52` | Executor JVM | Spark catch 到 OOM 异常，主动 System.exit(52) | 同 exit 143：调 memory.fraction / 减 cores / 加 memory |
| Executor 心跳失败退出（exit 56） | `exit code 56` / `Executor heartbeat timed out` | Executor → Driver | Executor 发送心跳失败主动退出，可能是 Driver GC/网络/压力大 | 排查 Driver 状态、增大 heartbeatInterval、检查网络 |
| Driver OOM（exit -104） | `exitCode: -104` / `AM Container ... timed out` | AM Container | Driver 堆内存溢出导致 AM 被 YARN kill | 调大 driver.memory 和 driver.memoryOverhead |
| Executor 被抢占 | `Container ... was preempted` | YARN NodeManager | 队列资源不足，Container 被 YARN 抢占回收 | 可忽略（Task 会自动重试），如频繁发生需排队列配额 |
| RECEIVED SIGNAL 15: SIGTERM | `RECEIVED SIGNAL 15: SIGTERM` | Executor | Dynamic Allocation 回收空闲 Executor / YARN 资源抢占 | 正常机制，无需处理；如导致 Shuffle 文件丢失需开启 ESS |

...
**[7.12 案例研究：大模型数据工程与评测岗 JD 拆解](./12-案例-大模型数据评测岗JD拆解.md)** —— 前面十一节讲「AI 工程怎么做」，这一节讲「拿这些能力去面一个具体岗位会遇到什么」。一家做「专家智能真身」的 AI 创业公司在招大模型数据+评测岗，围绕这个真实场景完成六件事：**招聘方尽调**（12 条宣传声明逐条核验，沉淀「早期公司尽调五查」方法论）→ **JD 逐条映射**知识体系（覆盖度 62%，缺口全在评测科学）→ **专家数字分身技术全景**（四层架构、PsyDT dynamic one-shot、多模型路由含金量评估）→ **领域知识**（中文心理/医疗数据集谱系、CharacterEval 13 指标、LLM-as-a-Judge 校准、医疗合规四条硬线）→ **20 道面试预测题与四张技术底牌** → **16 个反问 + 四维决策矩阵**。*适用人群：面试大模型数据/评测岗、或需要评估一家早期 AI 创业公司。*
...
- 如果你正在面试大模型数据/评测岗位，或想知道这一章的知识在真实 JD 里怎么被考察，直接读 **7.12**；它是一份完整的实战案例，也可以当作「如何对早期创业公司做尽调」的通用模板

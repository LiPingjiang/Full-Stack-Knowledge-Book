...
public class AsyncEmbeddingFunction extends RichAsyncFunction<Document, DocumentWithVector> {
    ...
    public asyncInvoke(Document doc, ResultFuture<DocumentWithVector> future) {
        ...
        if (!rateLimiter.tryAcquire()) {
            // 不阻塞——以异常完成，交由 timeout/异常回调去重试或入死信
...
**方法名里的 `wait` 是什么意思**——它指的是「**等待异步结果返回**」，而不是"等待重试"或"等待失败"。普通算子是同步的（一条处理完才处理下一条），Async I/O 则允许**同时发出多个请求**，`wait` 就是等这些"在飞"的请求回来：

```
同步（普通 map）：
  数据1 → 发请求 → 等 50ms → 得结果 → 数据2 → 发请求 → 等 50ms ...
  吞吐 = 1 / 50ms = 20 条/秒

异步（Async I/O，capacity=100）：
  数据1 → 发请求 ─┐
  数据2 → 发请求 ─┤  100 个请求同时在飞
  数据3 → 发请求 ─┤  谁先回来先处理谁
  ...          ─┘
  吞吐 = 100 / 50ms = 2000 条/秒
```

**`unorderedWait` vs `orderedWait`**——`wait` 前面的修饰词才是关键，它决定**结果按什么顺序发往下游**：

```
发出三个请求，返回耗时不同：
  A → 100ms、B → 20ms、C → 50ms

orderedWait：B 先回来也得在缓冲区里等着，因为 A 还没回
            → 输出 A → B → C（严格保持输入顺序）
            → 代价：一个慢请求阻塞后面所有已完成的结果（队头阻塞）

unorderedWait：谁先回来谁先发往下游
            → 输出 B → C → A，无队头阻塞，吞吐更高
```
...
> **为什么 Embedding 场景用 `unorderedWait`**：向量入库**不关心顺序**——每条数据独立写进向量库，先写后写没区别。而 Embedding 服务响应时间波动大，用有序等待会让一个慢请求拖垮整体吞吐。只有当下游是"按顺序更新某个状态"时才必须用 `orderedWait`。这和 [3.12 消息队列](../part3-java-deep/12-消息队列.md) 里 Kafka 的乱序消费换吞吐是同一个思路。
>
> 补充一个细节：`unorderedWait` 在有 Watermark 的场景下，**只在同一个 Watermark 区间内无序**，跨 Watermark 仍保持顺序，否则会破坏事件时间语义。

**`capacity` 才是背压开关**——上面代码里最后那个参数 `100`：

```
当在飞的请求达到 capacity 上限：
  → Flink 阻塞上游、停止读取新数据（反压）
  → 而不是丢弃数据或旁路
  → 等有请求返回腾出名额再继续
```

这和线程池的 `RejectedExecutionHandler` 不同——Flink 选择的是**让整条链路慢下来**，而不是丢弃。`capacity` 设多少取决于：**Embedding 服务能承受的并发数 ÷ Flink 并行度**。
...
第一级：重试 + 退避
  ...
  → 重试【发生在 asyncInvoke 内部】，该条数据的 ResultFuture 尚未 complete
  → 它只占用 capacity 的一个名额，主流继续处理其他数据，不阻塞

第二级：降级到该文档【上一版本】的向量
  → 按 doc_id（不是 content_hash！）取回这篇文档历史上算过的向量
  → 代价：内容已更新但向量是旧的 → 语义漂移小的修订可接受
  → 仅适用于【更新】场景；新增文档没有历史向量，只能进第三级

第三级：旁路到死信队列
  → 前两级都失败的文档 → 写入死信队列（Kafka topic / 侧输出）
  → 作业不阻塞，继续处理后续文档
  → 死信队列的文档由离线补偿任务重新处理
```

> **重试期间数据在哪里**——这里有个常见误解：**重试中的数据并不会进侧输出（Side Output）**。它就停留在 `asyncInvoke` 的回调链里，对应的 `ResultFuture` 还没有 `complete`，因此：
>
> - 它**占用 `capacity` 的一个名额**——这正是 Async I/O 的背压机制：在飞的请求达到 `capacity` 上限时，Flink 会阻塞上游读取，而不是丢数据
> - 它**不会流向下游任何分支**，因为异步结果只有 `complete` 之后才会被发射
> - Checkpoint 时这些**在飞的请求会被一并保存**，作业恢复后重新发起，所以不会丢
>
> **侧输出只在"放弃这条数据"时才用**——即三级容灾全部失败后。如果重试成功就走正常输出，两条路径互斥，不存在"重试成功了但侧输出里还有一份需要下游忽略"的问题。
>
> 换句话说：**重试是"还没出结果"，侧输出是"已经确定失败"**。前者是中间状态，后者是终态。
...
class EmbeddingPipeline:
    def __init__(self, service, cache, dlq_producer):
        ...
        self.pending_recompute = set()   # 用了 stale 向量、待服务恢复后补算的文档
    ...
    def embed(self, doc_id: str, content: str, content_hash: str) -> list[float] | None:
        # ① 内容哈希缓存命中（最常见的路径）—— 这是【去重】，不是降级
        #    命中条件：内容一模一样（如同一段落出现在多篇文档里）
        cached = self.cache.get(f"emb:content:{content_hash}")
        if cached:
            return cached
        ...
        try:
            ...
            self.cache.setex(f"emb:content:{content_hash}", 86400, vector)
            # 同时按 doc_id 记录"这篇文档最新的向量"，专供降级使用
            self.cache.set(f"emb:doc:{doc_id}", vector)
            return vector
        except Exception as e:
            # ③ 降级：取该文档【上一版本】的向量
            #    注意 key 是 doc_id 而非 content_hash——内容变了 hash 必然变，
            #    用新 hash 去查历史向量【永远不可能命中】
            stale = self.cache.get(f"emb:doc:{doc_id}")
            if stale:
                logger.warning(f"using stale embedding for {doc_id}: {e}")
                metrics.incr("embedding.degraded.stale")   # 必须打点监控降级比例
                self.pending_recompute.add(doc_id)          # 服务恢复后异步补算
                return stale

            # ④ 旁路到死信队列（新增文档没有历史向量，只能走这里）
            self.dlq.send({
                "doc_id": doc_id,
...
**两个缓存的分工必须分清**，这是本节最容易写错的地方：

| Key | 用途 | 命中条件 |
|-----|------|---------|
| `emb:content:{hash}` | **去重**——避免为相同内容重复计算 | 内容完全一致 |
| `emb:doc:{doc_id}` | **降级**——服务不可用时的兜底 | 这篇文档之前算过任意版本 |

> **为什么不能用 content_hash 做降级**：内容一改，hash 必然改变，用新 hash 查历史向量**永远不会命中**——降级逻辑会静默失效。这是一个很隐蔽的 bug，因为代码看起来是对的、也不报错，只是那个分支永远走不到。

**用旧向量的实际后果**——它只影响"能不能被检索到"，不影响"返回的内容对不对"：

```
文档 v1："订单超时时间是 30 分钟"    → 向量 V1（已在向量库）
文档 v2："订单超时时间改为 15 分钟"  → 服务挂了，降级沿用 V1

  · 用户搜"订单超时" → 仍能召回这篇文档（V1 语义够用）      ✓
  · 送给 LLM 的正文是 v2 最新内容（正文来自文档库，不受影响）✓
  · 只有当改动【显著改变语义】才会漏召回
    （如 v1 讲订单、v2 整篇改成讲支付 → 搜"支付"可能搜不到）✗
```

因为大多数文档更新是小幅修订（改数字、补说明），语义漂移很小，所以这个降级是可接受的。但**必须配套两件事**：打点监控降级比例、服务恢复后异步补算，否则这些文档会永久停留在旧向量上。

> **更进一步：新增和更新的降级策略应当不同。**
>
> ```
> 更新文档 → 向量库里【本来就有旧向量】
>            最简单的降级其实是【跳过这次写入】，什么都不做
>            —— 效果等同于"沿用旧向量"，但省掉一次写入
>
> 新增文档 → 没有任何历史向量可用
>            只能进死信队列，等服务恢复后补入
> ```
>
> 也就是说，对更新场景「用 stale 向量重新写一遍」和「直接跳过」是等效的，后者更简单。真正需要 stale 缓存的，是那些**必须保证写入链路不中断**的场景（比如下游依赖写入事件做触发）。

...
**什么是模块度（Modularity）**——社区划分得好不好，需要一个可优化的量化指标，模块度就是这个指标。它的定义很巧妙：
...
```
模块度 Q = （社区内实际的边数）−（随机连线时期望的边数）

  A_ij        ：节点 i 和 j 之间实际有没有边
  k_i·k_j/2m  ：如果把所有边打散随机重连，i 和 j 之间"本该"有多少边
  δ(c_i,c_j)  ：只统计同一社区内的节点对（不同社区的不计入）

取值范围 −0.5 ~ 1
  Q ≈ 0     → 这个划分和随机瞎分没区别，没意义
  Q > 0.3   → 社区结构显著
  Q 越高    → 社区内越紧密、社区间越稀疏
```

**为什么要减去"随机期望"**：不然会有个漏洞——把所有节点塞进一个大社区，社区内边数最多，但这显然不是有效划分。减去随机基线后，只有"比随机情况紧密得多"的分组才能得高分，从而避免了这种退化解。

**Louvain 与 Leiden 算法**——两者都是**贪心优化模块度**的启发式算法（精确求解是 NP 难问题，只能启发式逼近）：
...
Louvain 的两阶段迭代：
  ...
  阶段一：局部移动
    每个节点尝试移到邻居所在的社区，
    计算移动后模块度的增益 ΔQ，
    移到增益最大的那个社区（若都不增益则不动）
    → 反复扫描所有节点，直到没有节点再移动

  阶段二：图折叠
    把每个社区"压缩"成一个超级节点，
    社区间的边合并成带权边，社区内的边变成自环
    → 得到一张更小的图

  拿新图回到阶段一，继续迭代
  → 每轮迭代产生一个粒度层级，这就是【分层社区】的由来
```

**Leiden 修复了 Louvain 的一个严重缺陷**：Louvain 可能产出**内部不连通的社区**——某个节点被划入社区 A，但它与 A 中其他成员之间的连接路径在迭代中被切断了，结果这个"社区"在图上其实是分裂的几块。Leiden 增加了一步**细化（refinement）**，保证每个社区内部必然连通，同时收敛更快、结果更稳定。

> **对 GraphRAG 的实际影响**：社区不连通意味着摘要会把毫不相关的实体硬凑在一段文字里，直接污染 Global Search 的答案质量。所以 GraphRAG 明确选用 Leiden 而非 Louvain。代码里的 `random_seed=42` 也是同理——这类贪心算法对节点扫描顺序敏感，不固定种子会导致每次跑出的社区划分不同，摘要和检索结果就不可复现。
...
> **超级节点问题**：图谱里总有一些高度节点（比如"用户"这个实体可能连着上千条关系）。遍历时如果不限制，1 跳就能扩展出几千个节点，直接撑爆上下文。必须**按关系权重截断邻居数**。这和图数据库查询优化里处理 supernode（即上述超级节点）的思路一致。
...
> **GDS 是什么**：Graph Data Science，Neo4j 官方的图算法插件库，内置 Louvain/Leiden 社区发现、PageRank、最短路径、节点相似度等几十种算法，可直接用 Cypher 调用。对 GraphRAG 来说意义在于**社区发现不用自己实现、也不用把图导出到 Python 里算**。
...
| [7.11 检索平台工程](./11-检索平台工程.md) | GraphRAG 的成本与延迟在其容量规划和降级框架内管理 |

---

[← 7.9 AI 安全与可信](./09-AI安全与可信.md) | [返回本章目录](./README.md) | [7.11 检索平台工程 →](./11-检索平台工程.md)

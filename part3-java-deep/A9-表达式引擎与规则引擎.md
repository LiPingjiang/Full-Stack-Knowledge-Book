...
字节码编译型引擎（Aviator、Janino 等）的"编译"发生在什么时刻？编译后的产物能否缓存到外部存储？这是工程化落地的核心问题。

**编译时机：运行时惰性编译，首次执行时编译**

```
不是编译时（javac .java → .class）—— 表达式在运行时才以字符串形式存在
不是启动时（JVM 启动）—— 表达式可能从配置中心动态加载
而是首次执行时（runtime）—— 第一次碰到这个表达式字符串才编译

Aviator 三种调用方式：
  eval("a + b * c", env)
    → 编译 + 执行一步到位，不缓存 → 每次都重新编译

  compile("a + b * c")
    → 只编译不执行，返回 Expression 对象 → 手动持有和复用

  getCachedExpression("a + b * c")
    → 编译 + 存入内置 LRU 缓存
    → 首次调用编译，后续命中缓存直接返回
```
...
```
"a + b * c"（字符串）
  ↓ ExpressionLexer 词法分析
  Token 序列: [ID(a), OP(+), ID(b), OP(*), ID(c)]
  ↓ ExpressionParser 语法分析
  AST: AddExpr(a, MultExpr(b, c))
  ↓ CodeGenerator 代码生成（ASM 操作字节码）
  字节码: 对应一个 Java 类（继承 Expression）
  ↓ ClassLoader.defineClass()
  匿名类加载到 Metaspace（2~5KB）
  ↓ 实例化
  Expression 对象（在堆中，可被 GC 回收）
  ↓ execute(env)
  计算结果
```
...
```
层次一：不缓存（eval 方式）
  每次调用 → 重新编译 → 新匿名类 → Metaspace 持续增长
  → 生产环境严禁使用，只适合一次性脚本或测试

层次二：手动缓存（compile + 自行持有 Expression 对象）
  Expression expr = compile("a + b * c");  // 编译一次
  存入 Map<String, Expression> cache;
  每次从 cache.get() 获取 → 执行
  → 适合表达式集合固定且已知的场景

层次三：内置 LRU 缓存（getCachedExpression / enableCache）
  内部维护 ConcurrentHashMap<String, FutureTask<Expression>>
  → 首次调用：编译 + 存入缓存
  → 后续调用：命中缓存 → 直接返回 Expression
  → 缓存满时 LRU 淘汰最久未使用的条目
  → 用 FutureTask 保证并发安全（多线程同时首次访问同一表达式只编译一次）
```

生产环境的典型用法——启动时预热，运行时零编译开销：

```java
// 服务启动时预热——把配置文件中的表达式全部预编译
@PostConstruct
public void init() {
    List<String> expressions = loadFromConfig();  // 从 DB / 配置中心加载
    AviatorEvaluator.getInstance().enableCache();
    for (String exprStr : expressions) {
        AviatorEvaluator.getInstance().getCachedExpression(exprStr);  // 预编译
    }
}

// 运行时——零编译开销，只执行已缓存的字节码
public Object evaluate(String exprStr, Map<String, Object> env) {
    return AviatorEvaluator.getInstance()
        .getCachedExpression(exprStr)  // 命中缓存，不编译
        .execute(env);                 // 直接执行字节码
}
```

**编译产物能否缓存到外部存储（如 Redis）？**

不能。缓存的对象不是一个简单的数据，而是跟当前 JVM 进程绑定的"活对象"：

```
Aviator 缓存的是 Expression 对象：

  ① 类（AviatorJavaExpression_1703xxx）存在于当前 JVM 的 Metaspace 中
     → 由 Aviator 内部 ClassLoader 动态加载
     → 其他 JVM 里没有这个类

  ② Expression 对象在当前 JVM 堆中
     → 持有对 AST 节点、闭包、内部数据结构的引用
     → 这些引用指向的也是当前 JVM 中的对象

  ③ 不是 Serializable
     → Expression 类是 ASM 动态生成的，没有实现 Serializable
     → 即使强行序列化，反序列化的 JVM 找不到这个类
     → 抛出 ClassNotFoundException

  ④ 即使序列化 byte[] 本身（ASM 生成的字节码）
     → 生成的字节码引用了 Aviator 内部类，版本必须严格一致
     → 网络往返 I/O 开销可能抵消编译节省的时间
     → 没有主流引擎提供这个能力
```

正确的架构是：**外部缓存"源码"（表达式字符串），本地缓存"编译产物"（Expression 对象）**

```
Redis / DB / 配置中心（存表达式字符串——"源码"）：
  "feature_001": "add(multiply($user_age, 2), $base_score)"
  "feature_002": "log2($feature_value)"

每个 JVM 实例（本地内存缓存编译产物）：
  启动时：从 Redis 拉表达式字符串 → 本地 compile() → 缓存 Expression 对象
  运行时：直接执行本地缓存的 Expression（零网络 I/O）
  配置变更：Redis Pub/Sub 通知 → 本地缓存失效 → 重新拉取 + 重新编译

整个流程：
  Redis 存"配方"（字符串），每个 JVM 自己"炒菜"（编译）
  不能把"炒好的菜"存进 Redis（Expression 对象）
```

这个限制不是 Java 独有的，其他语言同理：

```
Python .pyc 文件——看似例外，实则同理：
  Python 的 compile() 生成 code object，可以 marshal 序列化到 .pyc 文件
  但 .pyc 只能在同一 Python 版本 + 同一字节码格式下使用
  跨版本（如 3.11 → 3.12）.pyc 不兼容，会自动重新编译

JavaScript V8 引擎：
  V8 编译的代码缓存在 V8 Isolate 内部，不能跨进程共享
  Chromium 有 V8 Code Cache（序列化编译结果到磁盘），但仅限同一浏览器实例

本质上：
  编译产物（字节码/机器码）跟运行时环境（ClassLoader/VM/版本）强绑定
  → 缓存作用域 = 进程级，不能跨进程
  → 外部缓存只能存"源码"（字符串），编译必须在本地做
```

### 3.5 Metaspace 清理机制——编译型引擎的隐性代价

字节码编译型引擎最大的工程化痛点：**堆中的 Expression 对象可以被 GC 正常回收，但 Metaspace 中的匿名类极难清理。**

**两层清理，机制完全不同**

```
第一层：堆中的 Expression 对象 → 正常 GC 回收
  缓存淘汰 → Expression 对象不再被引用 → Minor GC 正常回收 ✓
  跟普通 Java 对象一样，没什么特别的

第二层：Metaspace 中的匿名类 → 极难回收
  类卸载必须同时满足三个条件：
    ① 该类的 ClassLoader 不再被任何引用（ClassLoader 自身可被 GC）
    ② 该类没有任何活跃实例
    ③ 没有其他已加载类引用该类

  Aviator 用的是共享 ClassLoader：
    AviatorEvaluatorInstance（单例）
      → 持有 aviatorClassLoader（一个 ClassLoader 实例）
        → 加载了所有动态编译的表达式类
        → ClassLoader 被单例持有 → 永远可达 → 永远不会被 GC
        → 条件 ① 永远不满足
        → 所有匿名类永远无法卸载
        → Metaspace 单调增长，不可逆
```

LRU 缓存淘汰 ≠ Metaspace 回收：

```
编译 1000 个表达式 → Metaspace 增长 ~5MB（1000 × 2~5KB）
缓存淘汰 500 个旧的 → 堆中 500 个 Expression 对象被 GC ✓
                      → Metaspace 中的 500 个匿名类依然存在 ✗
                      → Metaspace 不降低

再编译 500 个新的 → Metaspace 再增长 ~2.5MB
                   → 总量从 5MB → 7.5MB（只增不减）
```
...
```
机制一：LRU 缓存淘汰（Aviator 内置）
  作用：控制 Expression 对象数量，限制堆内存
  局限：不清 Metaspace，只是让旧对象可被 GC
  → 缓解堆压力，不缓解 Metaspace 压力

机制二：Full GC + 类卸载
  触发条件：Metaspace 达到 MaxMetaspaceSize 阈值
  JVM 行为：Full GC 时尝试卸载不可达的类
  实际效果：ClassLoader 共享 → 条件不满足 → 类卸载不了
           → 最终抛 OutOfMemoryError: Metaspace

机制三：JVM 重启（终极手段）
  效果：进程退出 → Metaspace 全部释放
  实践：周期性滚动重启服务实例
  问题：影响可用性，治标不治本

机制四：替换 ClassLoader（Tomcat 热部署模式）
  思路：定期创建新 ClassLoader，废弃旧的
  效果：旧 ClassLoader 不可达 → Full GC 时卸载其加载的所有类
  实践：Tomcat 的 WebappClassLoader 就是这么做的
  局限：Aviator 原生不支持，需自研封装
  ...
  void refreshClassLoader() {
      ...
      currentLoader = new ExpressionClassLoader();  // 新 ClassLoader
      // 重新编译所有表达式，加载到新 ClassLoader
      recompileAll();
      // 旧 ClassLoader 不再被引用
      // 下次 Full GC 时，旧 ClassLoader 可被 GC
      // → 其加载的所有匿名类被卸载 → Metaspace 回收
  }
```
...
QLExpress 3.x（解释执行）：
  表达式字符串 → AST 对象（在堆中）
  缓存淘汰 → AST 对象失去引用 → Minor GC 正常回收 ✓
  Metaspace：零占用（不生成任何类）
  → 无类卸载问题，缓存淘汰即彻底清理

QLExpress 4.x（VM 指令）：
  表达式字符串 → 指令序列对象（在堆中）
  缓存淘汰 → 指令序列对象失去引用 → GC 正常回收 ✓
  Metaspace：零占用
  → 同样无此问题

Aviator（字节码编译）：
  表达式字符串 → 匿名类（Metaspace）+ Expression 对象（堆）
  缓存淘汰 → Expression 对象被 GC ✓
              → 匿名类留在 Metaspace ✗（共享 ClassLoader 不可达条件不满足）
  → 有类卸载问题
```
...
```
策略一：固定表达式集合（最推荐）
  启动时从配置中心加载所有表达式 → 预编译 → 锁定
  运行期间不动态生成新表达式
  Metaspace 用量 = 表达式数量 × 2~5KB，可预测
  → 1000 个表达式 ≈ 5MB Metaspace，完全可控

策略二：表达式去重
  不要动态拼接字符串生成"无限多"的不同表达式
  错误：eval("a + " + dynamicField + " * c")  → 每个不同值生成一个新类
  正确：eval("a + $field * c", env)          → 一个表达式，field 通过 env 传入

策略三：版本化全量替换
  规则变更时：创建新 ClassLoader → 重新编译全部 → 替换缓存
  旧 ClassLoader 废弃 → 下次 Full GC 时 Metaspace 回收
  → 类似 Tomcat 热部署，需要自研封装

策略四：设 MaxMetaspaceSize + 监控告警
  -XX:MaxMetaspaceSize=512m
  监控 jvm.memory.metaspace.used
  超过 80% 告警 → 排查是否有动态表达式生成
  达到 100% → OutOfMemoryError: Metaspace → 服务崩溃

策略五：解释型兜底
  当表达式数量不可控（用户自定义、规则引擎前端输入）
  → 用 QLExpress（解释执行），牺牲性能换内存安全
  → 不用 Aviator（编译执行），避免 Metaspace 泄漏
```

> **核心矛盾**：编译型引擎用 Metaspace 换速度（快但难清理），解释型引擎用速度换内存安全（慢但干净）。LRU 缓存淘汰只解决堆内存问题，Metaspace 的清理依赖 ClassLoader 生命周期管理，这是编译型引擎的固有代价。

...
## 七、SDK 工程化：把内部能力变成稳定产品接口

SDK 不是把 HTTP 请求简单包一层，而是把认证、序列化、重试、超时、错误语义、版本演进和可观测性收敛为稳定的开发者接口。KMS、支付、存储、AI 等平台都依赖 SDK 降低接入方的复杂度。

### 7.1 SDK 分层与设计模式映射

SDK 的公共 API 应尽量小而稳定，底层传输与领域模型可独立演进。典型结构是：`Client` 负责生命周期和统一配置，`Service` 按业务域划分能力，`Request/Response` 表示协议模型，`Transport` 隐藏 HTTP/gRPC 细节，`CredentialProvider` 提供认证材料。

| SDK 问题 | 推荐模式 | 作用 |
|---------|---------|------|
| 多种认证方式（AK/SK、OAuth、mTLS） | 策略 + 工厂 | 运行时选择凭证实现，避免 Client 内堆积 if-else |
| 兼容旧接口或旧协议 | 适配器 | 新实现不破坏已有调用方 |
| 构造复杂 Client | 建造者 | 以链式 API 配置 endpoint、超时、重试、凭证 |
| 隐藏多个下游服务 | 外观 | 用一个业务 API 屏蔽鉴权、签名、请求发送和解析 |
| 重试、日志、指标、签名 | 装饰器 / 责任链 | 在不污染业务代码的前提下叠加横切能力 |
| 可替换传输实现 | 依赖倒置 | 面向 `Transport` 接口编程，便于 HTTP/gRPC/Mock 切换 |

Java 风格的入口通常是：`KmsClient.builder().endpoint(...).credentialProvider(...).retryPolicy(...).build()`。建造者负责在构造期校验必填配置；调用期参数校验、签名、超时和重试则应位于拦截器链中。

### 7.2 版本、错误与可靠性边界

SDK 对外遵守语义化版本：补丁版本只修 Bug，次版本新增向后兼容能力，主版本才允许破坏性变更。服务端 API 应先新增字段和能力，再让旧字段经历弃用期；JSON 忽略未知字段，Protobuf 保留已废弃 field number，避免客户端和服务端灰度期间互相不兼容。

错误不能把底层异常原样抛给用户。应建立稳定的领域异常层，例如 `AuthenticationException`、`PermissionDeniedException`、`RateLimitException`、`ConflictException`、`ServiceUnavailableException`，并保留 request_id、错误码和可安全展示的诊断信息。网络超时、连接重置、429/503 可以在满足幂等条件时做指数退避重试；创建、扣款、轮换密钥等非幂等操作必须使用 idempotency key，不能盲目自动重试。

### 7.3 SDK 的安全与可观测性

凭证不能写入日志、异常消息或埋点标签；SDK 应支持环境变量、配置文件、实例身份或外部凭证提供者，避免业务代码散落明文 AK/SK。对 KMS 一类高安全服务，SDK 通常还需要请求签名、TLS/mTLS、时钟偏差处理和证书轮换。

每次调用至少应暴露请求耗时、结果码、重试次数和 request_id，支持将 trace context 透传到服务端。这样接入方遇到失败时能通过 request_id 与服务端审计日志、链路追踪关联排查。

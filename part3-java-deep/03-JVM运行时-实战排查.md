...
**没有图形界面**（服务器 / sandbox / 远程终端）怎么办？用 JDK 8 自带的 `jhat` 直接分析 hprof：

```bash
# 启动 jhat，把 hprof 解析成一个本地网页服务（默认 7000 端口）
jhat -port 7000 heap.hprof

# 看到 "Server is ready." 后，访问以下页面（端口转发或本机浏览器）：
#   http://localhost:7000/             首页，最下方有各种分析入口
#   http://localhost:7000/histo/       对象直方图：按占用排序，[B 一般排最前
#   http://localhost:7000/showInstanceCounts/   按实例数排序
# jhat 还支持 OQL（Object Query Language）做对象检索
```

> `jhat` 胜在「随 JDK 自带、纯命令行可起」，缺点是分析能力比 MAT 弱、大 dump 慢。**所以更推荐：与其事后啃 hprof，不如趁进程活着时用 `jmap -histo:live <pid> | head -20` 直接看直方图**——一眼就能看到 `[B`（byte 数组）占了绝大部分，正好对上代码里的 `new byte[1024*1024]`，泄漏对象立刻锁定。
...
| 无界面分析 hprof | `jhat -port 7000 heap.hprof` | JDK 自带，浏览器看直方图/引用链 |

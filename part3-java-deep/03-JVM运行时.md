...
```
1. 类加载检查（类没加载先触发加载）
2. 分配内存（指针碰撞 or 空闲列表，取决于堆是否规整）
   └─ 并发安全：CAS 重试 或 TLAB（线程本地分配缓冲，每个线程一小块，无竞争）
3. 初始化零值（字段默认值 0/null）
4. 设置对象头（Mark Word：hashCode/GC分代年龄/锁状态；类型指针）
5. 执行 <init> 构造方法（程序员定义的初始化）
```
...
```
1. jstat -gcutil <pid> 1000   # 看 GC 频率和各区占用，确认是不是 FullGC 频繁
2. jmap -dump:live,format=b,file=heap.hprof <pid>   # dump 堆
   （或靠 -XX:+HeapDumpOnOutOfMemoryError 自动 dump）
3. 用 MAT / JProfiler 分析 hprof，找 "Dominator Tree" 里最大的对象
4. 定位到泄漏的对象 → 找引用链 → 锁定代码（常见：静态集合只加不删、
   ThreadLocal 没 remove、连接未关闭、缓存无淘汰）
```

**CPU 飙高排查**：

```
1. top -Hp <pid>           # 找到占 CPU 最高的【线程】tid
2. printf "%x\n" <tid>     # 线程 id 转 16 进制
3. jstack <pid> | grep <16进制tid> -A 30   # 在线程栈里定位到具体代码行
   （常见：死循环、正则回溯、频繁 GC、锁竞争自旋）
```

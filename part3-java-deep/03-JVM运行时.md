...
> ⚠️ **别掉进「RT 高 = CPU 忙」的思维定式**：CPU 飙高只是「接口变慢」的一种根因。另一大类更隐蔽——`top` 看 CPU 一点都不高，但接口 RT 明显变长甚至超时，往往是线程在**傻等**：要么大量线程 `BLOCKED` 在抢同一把锁，要么线程池被打满、请求在队列里排队。这类问题要用 `jstack` 统计线程状态分布、比对 `waiting to lock`/`locked` 地址找到持锁元凶，或者看线程池的 `getActiveCount()`/`getQueue().size()`——和「找烧 CPU 的线程」是完全不同的排查路径，详见配套实战附录。

> 📌 **想要可复现的示例代码 + 一条条照着敲的命令**？见配套实战附录：[3.3 附录 · 线上 OOM / CPU 飙高 / 锁竞争实战排查](./03-JVM运行时-实战排查.md)（含 OOM 泄漏复现代码、CPU 飙高复现代码、锁竞争与线程池打满复现代码、完整排查命令、Arthas 用法、命令速查表）。

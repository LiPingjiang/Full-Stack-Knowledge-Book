...
| 方法区/元空间 | 共享 | `OutOfMemoryError: Metaspace`（动态生成类过多） |
...
> **追问：对象一定分配在堆上吗？** 不一定！**逃逸分析**优化下，如果对象不会逃出方法作用域，JVM 可能做**栈上分配**（随栈帧回收，无 GC 压力）或**标量替换**（把对象拆成基本类型直接放栈）。这是高级加分点，也呼应了第四章 [Go 的逃逸分析](../part4-multilang-compare/03-Java到Go.md)。

### 考点 3：GC 怎么判断对象存活 + 引用类型

**面试官**：「怎么判断对象该不该回收？为什么不用引用计数？」

- **不用引用计数**的原因：**无法解决循环引用**（A 引用 B，B 引用 A，互相引用计数都不为 0，但实际已无人使用，永不回收）。
- 用 **可达性分析**（GC Roots 出发，不可达即回收）。**GC Roots 包括**：虚拟机栈中引用的对象、静态变量引用的对象、常量引用、本地方法栈 JNI 引用、活跃线程。
...
- 基础算法：**标记-清除**（产生碎片）、**标记-复制**（新生代用，空间换效率）、**标记-整理**（老年代用，无碎片但慢）。
- **三色标记**（并发标记的核心）：把对象标记为白（未访问）、灰（自己被访问、引用未扫完）、黑（自己和引用都扫完）。

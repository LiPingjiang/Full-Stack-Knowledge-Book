...
<details>
<summary><b>展开：解释执行和 JIT 都是跑机器码，到底差在哪？没有「远程机器码」吧？</b></summary>

先破一个常见误解：**CPU 只认本地机器码，没有什么「远程机器码」**。解释执行和 JIT 编译，最终都是在本机 CPU 上执行本机机器指令——这一点完全一致。它们的区别**不在「跑不跑机器码」，而在「字节码 → 机器指令的转换发生在什么时机、以什么粒度、转换结果有没有被保存复用」**。
...
> 打个比方：**解释执行像拿着字典逐句口译，每句都现查，说完就忘**；**JIT 像发现某段话天天要翻，干脆整段译好背下来，以后张口就来还越说越溜**。两者说的都是「本地机器码」这门语言，差别只是一个临场零碎翻译不留稿、一个整段预译并缓存复用。
>

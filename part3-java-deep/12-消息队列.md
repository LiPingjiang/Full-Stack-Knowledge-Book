...
### 2.6 Consumer Rebalance 深度机制（面试高频追问）

**Rebalance（重平衡）**：当消费者组内成员发生变化时，Kafka 重新分配 Partition 与 Consumer 的对应关系。
...
① 消费者组成员变化
   → 新消费者加入（扩容）
   → 消费者主动离开（正常下线，发送 LeaveGroup）
   → 消费者崩溃（心跳超时，Coordinator 被动检测）

② 订阅的 Topic 分区数变化
   → Topic 新增了 Partition

③ 订阅关系变化
   → 消费者组内某个成员修改了订阅的 Topic 列表
```

#### Rebalance 的三阶段流程

```
阶段一：Find Coordinator
  → 消费者向任意 Broker 发送 FindCoordinator 请求
  → 找到负责该 Consumer Group 的 GroupCoordinator（按 group.id hash 到 __consumer_offsets 的某个 Partition 的 Leader）

阶段二：Join Group
  → 所有消费者向 Coordinator 发送 JoinGroup 请求
  → Coordinator 选出一个 Consumer 作为 Leader（通常是第一个加入的）
  → Leader 负责制定分区分配方案

阶段三：Sync Group
  → Leader 将分配方案通过 SyncGroup 请求发给 Coordinator
  → Coordinator 将方案下发给所有消费者
  → 每个消费者拿到自己负责的 Partition 列表，开始消费
```
...
| 策略 | 原理 | 优缺点 |
|------|------|--------|
| **Range**（默认） | 按 Topic 维度，将 Partition 按序号范围分给 Consumer | 简单，但多 Topic 时容易不均匀（前面的 Consumer 总是多分一个） |
| **RoundRobin** | 将所有 Topic 的所有 Partition 打散，轮询分配 | 更均匀，但要求组内所有 Consumer 订阅相同的 Topic |
| **Sticky**（推荐） | 在 RoundRobin 基础上，尽量保持上次的分配结果不变 | 减少不必要的 Partition 迁移，降低 Rebalance 开销 |

#### Rebalance 的代价与优化

```
Rebalance 的代价：
  ① STW 效应：Rebalance 期间所有消费者停止消费（类似 GC 的 Stop-The-World）
  ② 重复消费：Rebalance 后从上次提交的 Offset 开始，已处理但未提交的消息会被重复消费
  ③ 耗时长：大规模集群（数百消费者）Rebalance 可能持续数分钟

避免不必要 Rebalance 的参数调优：
  → session.timeout.ms = 25000（默认 10s 太短，网络抖动容易误判）
  → heartbeat.interval.ms = session.timeout.ms / 3 = 8000
  → max.poll.interval.ms = 300000（处理逻辑重时调大，避免被踢）
  → 确保消费逻辑不要超过 max.poll.interval.ms

Kafka 2.4+ 的改进——Static Membership：
  → 设置 group.instance.id 为固定值
  → 消费者短暂离线再上线时，不触发 Rebalance（直接恢复原来的分配）
  → 适合容器化部署（Pod 重启不触发全组 Rebalance）
```

### 2.7 HW/LEO 副本同步机制——数据一致性的底层保障

**核心概念**：

```
LEO（Log End Offset）：每个副本的日志末端偏移量，即下一条待写入消息的位置
HW（High Watermark，高水位）：所有 ISR 副本中最小的 LEO

关系：
  → Consumer 只能消费 HW 之前的消息（已提交的消息）
  → HW 之后、LEO 之前的消息对 Consumer 不可见（未提交）

示例：
  Leader  LEO = 10, HW = 7
  Follower1 LEO = 8
  Follower2 LEO = 7  ← 最慢的 ISR 副本决定了 HW

  Consumer 只能读到 Offset 0~6 的消息（HW=7 意味着 0~6 已提交）
```

**副本同步流程**：

```
1. Producer 发消息到 Leader，Leader 写入本地 Log，LEO 递增
2. Follower 向 Leader 发送 Fetch 请求，携带自己的 LEO
3. Leader 返回数据，同时根据所有 ISR 副本的 LEO 更新 HW
4. Follower 写入本地 Log，更新自己的 LEO 和 HW
5. 当所有 ISR 副本都同步到某条消息后，该消息的 Offset < HW，对 Consumer 可见
```

**Leader Epoch 机制（Kafka 0.11+ 解决 HW 截断问题）**：

```
问题：旧版本中，Follower 重启后会根据 HW 截断日志，可能导致数据丢失

场景：
  Leader HW=1, LEO=2（刚写入一条但 Follower 还没同步）
  Follower 重启 → 按 HW=1 截断 → Leader 也挂了
  Follower 成为新 Leader → 那条消息永久丢失

解决：Leader Epoch = (epoch, startOffset) 对
  → 每次 Leader 切换，epoch +1
  → Follower 重启后不再盲目按 HW 截断，而是向 Leader 查询 epoch 对应的 endOffset
  → 只截断确实不属于当前 epoch 的数据
```

**面试回答模板**：

> Kafka 通过 HW 机制保证消费者只能读到已提交（所有 ISR 副本都同步）的消息。LEO 是每个副本的写入进度，HW 是所有 ISR 中最小的 LEO。0.11 版本引入 Leader Epoch 解决了 HW 截断导致的数据丢失问题。

### 2.8 Controller 与 KRaft 架构演进

#### Controller 的职责

```
Controller 是 Kafka 集群中的"大脑"，由某个 Broker 担任（通过 ZK 选举）：

核心职责：
  ① Partition Leader 选举：Broker 宕机时，为其上的 Partition 选新 Leader
  ② 集群元数据管理：维护 Topic/Partition/Replica 的分配信息
  ③ ISR 变更通知：某个副本被踢出/加入 ISR 时，通知相关 Broker
  ④ Topic 创建/删除：处理 AdminClient 的管理请求
  ⑤ Preferred Leader 选举：定期将 Partition Leader 迁回首选副本，均衡负载

Controller 故障转移：
  → Controller 所在 Broker 宕机
  → 其他 Broker 通过 ZK 竞争 /controller 临时节点
  → 新 Controller 从 ZK 加载全量元数据，重建内存状态
  → 向所有 Broker 发送 LeaderAndIsr + UpdateMetadata 请求
```
...
```
ZooKeeper 的问题：
  ① 运维负担：需要独立部署和维护 ZK 集群（3~5 节点）
  ② 扩展瓶颈：ZK 的 Watch 机制在大规模集群（数万 Partition）下性能下降
  ③ Controller 重启慢：需要从 ZK 全量加载元数据，大集群可能耗时数分钟
  ④ 脑裂风险：ZK 和 Kafka 是两套分布式系统，网络分区时可能出现不一致
  ⑤ 元数据不一致：Controller 内存状态与 ZK 存储可能短暂不一致
```

#### KRaft 模式（Kafka 3.3+ 正式 GA）

```
KRaft = Kafka + Raft 共识协议

核心变化：
  → 去掉 ZooKeeper，元数据由 Kafka 自己管理
  → 引入 Controller Quorum（3~5 个 Controller 节点组成 Raft 组）
  → 元数据存储在内部 Topic __cluster_metadata 中（Raft 日志）

架构对比：
  旧架构：Broker → ZooKeeper（元数据） + Controller（单点）
  新架构：Broker → Controller Quorum（Raft 多副本，内置在 Kafka 进程中）

KRaft 的优势：
  ① 部署简化：不再需要独立的 ZK 集群
  ② 启动更快：Controller 从本地 Raft 日志恢复，无需全量加载
  ③ 扩展性强：元数据变更通过 Raft 日志增量同步，支持百万级 Partition
  ④ 一致性更好：单一来源（Raft 日志），不存在 ZK 与内存状态不一致
  ⑤ Controller 故障转移更快：Raft 选举通常在秒级完成

迁移路径：
  Kafka 3.3：KRaft 正式 GA，新集群推荐直接用 KRaft
  Kafka 3.5+：提供 ZK → KRaft 在线迁移工具
  Kafka 4.0（规划）：彻底移除 ZooKeeper 依赖
```

**面试回答模板**：

> Kafka 早期依赖 ZooKeeper 做元数据管理和 Controller 选举，但 ZK 带来了运维复杂度和扩展瓶颈。Kafka 3.3 引入 KRaft 模式，用内置的 Raft 共识协议替代 ZK，Controller 节点组成 Quorum，元数据存在内部 Topic 中通过 Raft 日志同步。好处是部署简化、启动更快、支持更大规模集群。
...
```
一个 Partition 在磁盘上是一个目录，内部按 Segment 切分：

topic-0/
├── 00000000000000000000.log      ← 消息数据文件（追加写）
├── 00000000000000000000.index    ← 偏移量索引（稀疏索引，每隔 4KB 记一条）
├── 00000000000000000000.timeindex ← 时间戳索引
├── 00000000000005367851.log      ← 第二个 Segment（文件名 = 该 Segment 第一条消息的 Offset）
├── 00000000000005367851.index
└── 00000000000005367851.timeindex

查找 Offset=5370000 的消息：
  ① 二分查找 Segment 文件名 → 定位到 00000000000005367851.log
  ② 在 .index 中二分查找 → 找到最近的索引条目（如 Offset=5369990, Position=12345）
  ③ 从 Position=12345 开始顺序扫描 .log 文件 → 找到目标消息
```
...
| 策略 | 配置 | 适用场景 |
|------|------|---------|
| **Delete**（删除） | `log.retention.hours=168`（7天）<br>`log.retention.bytes=-1`（不限大小） | 日志流、事件流（过期即删） |
| **Compact**（压缩） | `log.cleanup.policy=compact` | 状态变更流（只保留每个 Key 的最新值） |
...
压缩后（只保留每个 Key 的最新 Value）：
  K1:V3 → K3:V4 → K2:V5

适用场景：
  → 数据库 CDC 变更流（只关心最新状态）
  → 配置变更通知（只需要最新配置）
  → __consumer_offsets 内部 Topic（只需要最新提交的 Offset）
```
...
| MQ 选型 | 日志流选 Kafka，业务消息选 RocketMQ，轻量路由选 RabbitMQ，进程间通信选 ZeroMQ，计算存储分离选 Pulsar |
...
## 十、Pulsar — 计算存储分离的消息队列

前面讲的 Kafka 是计算存储耦合架构——Broker 既负责消息路由又负责持久化存储（Partition 物理在 Broker 上），扩容需要数据迁移。Pulsar 采用了不同的架构——计算存储分离。

### 10.1 架构差异

Pulsar 的分层架构：Broker 层负责消息路由和协议处理（无状态，可以水平扩容，不存数据），BookKeeper 层负责持久化存储（BookKeeper 是分布式日志存储，类似一个 append-only 的分布式文件系统）。Broker 和 BookKeeper 可以独立扩缩容——流量大但存储少时只扩 Broker，存储增长快时只扩 BookKeeper。

Kafka 的耦合架构：Broker 既负责路由又负责存储。Partition 物理绑定在 Broker 上，扩容时需要 Rebalance Partition（数据从旧 Broker 迁移到新 Broker），迁移过程中影响吞吐。Kafka 的 Partition 是最小的存储和并行度单位——一个 Partition 只能在一个 Broker 上。

Pulsar 的解耦架构：Topic 是逻辑概念，Topic 的 Partition 可以分布在多个 Broker 上，每个 Partition 的数据存储在多个 BookKeeper Bookie 上。扩容 Broker 时不迁移数据（因为 Broker 无状态），只迁移 Partition 的 ownership（元数据级别的操作，秒级完成）。

### 10.2 订阅模式

Pulsar 提供四种订阅模式（Kafka 只有 Consumer Group 内的 Partition 分配）：Exclusive（独占——同一时刻只有一个 Consumer 消费一个 Topic）、Shared（共享——多个 Consumer 轮询消费，消息无序但高吞吐）、Failover（故障转移——主备模式，主 Consumer 消费，备 Consumer 待命）、Shared（Key_Shared——按 Key 分配给固定 Consumer，保证同 Key 消息顺序）。

### 10.3 Kafka vs Pulsar 选型

| 维度 | Kafka | Pulsar |
|------|-------|--------|
| 架构 | 计算存储耦合 | 计算存储分离 |
| 扩容 | 需要数据迁移（分钟级） | 只迁移 ownership（秒级） |
| 存储 | Broker 本地磁盘 | BookKeeper 分布式日志 |
| 订阅模式 | Consumer Group | Exclusive/Shared/Failover/Key_Shared |
| 多租户 | 弱（需要集群隔离） | 原生多租户（Topic 级隔离） |
| 延迟消息 | 不原生支持 | 原生支持（Delayed Delivery） |
| 生态 | 最成熟，Hadoop/Spark/Flink 原生集成 | Growing，Flink/Spark connector 可用 |

选型建议——高吞吐日志流场景选 Kafka（生态最成熟，运维经验积累最多），需要灵活订阅模式、多租户隔离、计算存储独立扩展的场景选 Pulsar（如 SaaS 平台的消息服务、自动驾驶平台的任务事件流）。

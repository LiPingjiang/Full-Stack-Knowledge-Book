...
Kafka 最初的定位是**分布式日志系统（Distributed Commit Log）**，不是传统消息队列。它的核心设计目标是**超高吞吐**，为此在架构上做了四个关键选择：
...
这四大法宝不是 Kafka 独有的，其他 MQ 也部分采用，但各家的设计取舍不同，导致性能特征差异很大：

| 维度 | Kafka | RocketMQ | RabbitMQ |
|------|-------|----------|----------|
| **定位** | 分布式日志系统，后被当作 MQ 使用 | 专为业务消息设计的 MQ | 传统消息中间件（AMQP 协议） |
| **单机吞吐** | **百万级** msg/s | **十万级** msg/s | **万级** msg/s |
| **延迟** | 毫秒级（批量攒够才发，有一定延迟） | 毫秒级（实时发送，延迟更低） | 微秒级（单条即发，延迟最低） |
| **顺序写** | ✅ 追加写日志 | ✅ 追加写 CommitLog | ❌ 随机写（Erlang Mnesia 存储） |
| **零拷贝** | ✅ sendfile | ✅ mmap | ❌ |
| **批量发送** | ✅ 默认攒批 | ⚠️ 支持但默认单条 | ❌ 单条发送 |
| **分区并行** | ✅ Partition | ✅ MessageQueue | ❌ Queue 级别串行 |
| **适用场景** | 日志采集、大数据管道、流处理 | 电商交易、金融业务消息 | 中小规模、需要复杂路由的场景 |

> Kafka 吞吐量碾压其他 MQ 的根本原因：它把消息当**日志**来处理——只追加不修改、批量攒够再发、分区并行读写。代价是牺牲了部分消息特性（如延迟队列、消息优先级等需要额外方案来实现）。RocketMQ 在吞吐和功能之间取了折中，RabbitMQ 则是功能最丰富但吞吐最低。

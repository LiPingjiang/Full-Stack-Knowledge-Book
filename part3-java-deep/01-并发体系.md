...
| 5 | `workQueue` | `BlockingQueue<Runnable>` | **任务队列**。核心线程都忙时，新任务先进队列排队**等待**，而不是立刻开新线程。队列类型决定了线程池的「性格」（见下表）。**必须用有界队列**，否则堆积会 OOM。 |
...
- **CAS（Compare-And-Swap）**：靠一条 CPU 原子指令实现「比较并交换」，是乐观锁、自旋锁、原子类的底座（详见后面的考点 4）。
- **`synchronized`**：靠对象头 Mark Word 记录锁状态，按竞争程度在偏向锁→轻量级锁（CAS 自旋）→重量级锁（OS 互斥量）之间升级（详见考点 3）。
...
`ThreadPoolExecutor` 七个参数：`corePoolSize`、`maximumPoolSize`、`keepAliveTime`、`unit`、`workQueue`、`threadFactory`、`handler`（拒绝策略）。每个参数的具体作用见本章第二节的逐参数详解。

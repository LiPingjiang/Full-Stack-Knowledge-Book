...
| **能否重入** | 可重入锁 (Reentrant) / 不可重入锁 (Non-reentrant) | 可重入：同一线程可重复获取同一把锁不死锁；[不可重入：同一线程再次申请会把自己锁死](#不可重入锁示例) | `synchronized`、`ReentrantLock` 都可重入 |
| **是否可共享** | 独占锁 (Exclusive) / 共享锁 (Shared) | 独占：一次一个线程；共享：可多个线程同时持有 | 独占=写锁；共享=读锁、`Semaphore`、`CountDownLatch` |

<a id="不可重入锁示例"></a>

<details>
<summary><b>👉 「不可重入锁」会怎样？一个自我死锁的可运行例子（点击展开）</b></summary>

**可重入（Reentrant）** 的意思是：**同一个线程已经持有这把锁后，再次申请同一把锁能直接拿到**（锁内部记一个「重入计数」，加锁 +1、解锁 -1，归零才真正释放）。`synchronized` 和 `ReentrantLock` 都是可重入的——这正是它名字里 "Reentrant" 的由来。

反过来，**不可重入锁** 不认「这是不是我自己」，只要锁被占着，再来申请就得排队等。于是出现一个荒谬的局面：**线程把自己锁死了**——它在等一把锁释放，而那把锁恰恰攥在它自己手里。JDK 没有现成的不可重入锁，我们手写一个最简版来复现：

```java
public class NonReentrantDemo {

    // 手写一个【不可重入】锁：只记「有没有被占」，不认持有者是谁
    static class NonReentrantLock {
        private boolean locked = false;
        public synchronized void lock() throws InterruptedException {
            while (locked) {        // 已被占用就一直等——哪怕占用者就是我自己
                wait();
            }
            locked = true;
        }
    ...
    static void outer() throws InterruptedException {
        lock.lock();
        System.out.println("进入 outer，已拿到锁");
        inner();                    // 同一线程里再调用 inner，又要拿同一把锁
        lock.unlock();
    }
    static void inner() throws InterruptedException {
        lock.lock();                // ❌ 卡死在这：锁被「我自己」占着，永远等不到释放
        System.out.println("进入 inner");  // 这行永远打印不出来
        lock.unlock();
    }

    public static void main(String[] args) throws InterruptedException {
        outer();   // 打印「进入 outer」后就卡死，发生自我死锁
    }
}
```

把上面的 `NonReentrantLock` 换成 JDK 的可重入锁，同样的「方法套方法」调用就完全正常——因为它认得「这是同一个线程」，重入计数 +1 直接放行：

```java
import java.util.concurrent.locks.ReentrantLock;

public class ReentrantDemo {
    static final ReentrantLock lock = new ReentrantLock();  // 可重入

    static void outer() {
        lock.lock();
        try {
            System.out.println("进入 outer，重入次数=" + lock.getHoldCount());  // 1
            inner();                 // 同一线程再次加锁，不会死锁
        } finally {
            lock.unlock();
        }
    ...
    static void inner() {
        lock.lock();                 // ✅ 同一线程重入，直接拿到，计数变 2
        try {
            System.out.println("进入 inner，重入次数=" + lock.getHoldCount());  // 2
        } finally {
            lock.unlock();           // 计数减回 1，outer 的 unlock 再减到 0 才真正释放
        }
    }
    public static void main(String[] args) {
        outer();   // 正常打印：进入 outer(1) → 进入 inner(2)
    }
}
```

**一句话总结**：可重入锁靠「**记住持有者 + 重入计数**」化解了「自己等自己」的死锁，这对**递归方法、一个加锁方法里调用另一个加锁方法**（在真实代码里极其常见）至关重要——这也是为什么 Java 默认的 `synchronized` 和 `ReentrantLock` 都做成可重入的。
...
> **「偏向」到底是什么意思？英文是 Biased Locking（偏向锁）/ Biased Towards（偏向于）。** 这里的「偏向」就是日常说的「偏心、偏袒」——锁**偏袒第一个获得它的线程**。背景是 HotSpot 团队发现一个统计规律：**绝大多数锁在整个生命周期里其实只被同一个线程反复获取**，根本没有竞争（比如 `StringBuffer`、`Vector` 这种内部加了锁、但实际只在单线程里用的对象）。既然如此，每次进出 `synchronized` 都做一次 CAS 加锁/解锁就太浪费了。
> 于是「偏向锁」的做法是：**第一次有线程进来时，把这个线程的 ID 直接写进对象的 Mark Word**（对象就此「认准」了这个线程、对它「偏心」）。**之后只要还是同一个线程来加锁，只需检查 Mark Word 里的线程 ID 是不是自己——是就直接进，连一次 CAS 都不用做**，几乎零开销。这就是「偏向」省事的本质：用「记住上一个持有者」赌「下一个还是它」。
> 一旦赌输了——**有第二个线程也来抢这把锁**，偏向的前提（无竞争）就被打破，锁会**撤销偏向（revoke bias）**并升级为轻量级锁，开始真正的 CAS 竞争。正因为「撤销偏向」本身代价不小、在高并发下收益反而不稳定，**JDK 15 起官方默认禁用并逐步废弃了偏向锁**（[JEP 374](https://openjdk.org/jeps/374)）。
...
<details>
<summary><b>展开：「阻塞了不就走不到 interrupt 了吗？」——澄清一个常见困惑</b></summary>

很多人第一次看到这条规则会卡住：「线程都阻塞了，怎么还能执行 `interrupt()` 抛异常？」

**症结在于：`interrupt()` 不是阻塞的那个线程自己调用的，而是另一个线程去调用它的。** 「谁阻塞」和「谁发中断」是**两个不同的线程**。

- 线程 A 正卡在 `Thread.sleep(10000)` 上睡觉（阻塞、让出 CPU、停在那行不往下走）。
- 线程 B（比如主线程）一直清醒地运行着，它执行 `threadA.interrupt()`——B 没阻塞，所以这句它当然能执行。
- B 这一戳能**穿透 A 的阻塞**：A 会**立刻从睡眠中被唤醒**，并在 `sleep` 那一行**抛出 `InterruptedException`**（不会睡满 10 秒）。

```java
Thread a = new Thread(() -> {
    try {
        Thread.sleep(10000);            // A 卡在这里睡 10 秒
    } catch (InterruptedException e) {
        System.out.println("被叫醒了，没睡满！"); // ← B 一 interrupt，这里立刻执行
    }
});
a.start();
Thread.sleep(1000);                      // 主线程 B 正常运行
a.interrupt();                           // B 对 A 喊：醒醒！→ A 的 sleep 立刻抛异常
```

**为什么叫「可中断方法」？** 准确叫法是「**可中断的阻塞方法 / 响应中断的方法**」（interruptible blocking methods）。不是因为它们代码里设置了中断位，而是因为它们在挂起线程时是「**可被中断唤醒**」的——一旦有人对该线程发中断，它们能立刻被唤醒并以抛 `InterruptedException` 的方式响应。对比之下，抢 `synchronized` 锁时的阻塞、大多数普通 I/O 阻塞**不响应中断**，所以它们不算「可中断方法」。

**一句话**：A 在可中断方法里睡着 → B（清醒着）对 A 喊 `interrupt()` → A 立刻被叫醒并在那行抛 `InterruptedException`。这就是「优雅、及时地叫停一个正在阻塞等待的线程」的机制，不用干等它自己醒。
...
一句话抓住本质：JVM 把大量虚拟线程「挂载」到少量 OS 线程（carrier thread）上做用户态调度，阻塞 IO 时自动把虚拟线程卸载、让 OS 线程去跑别的，于是 Java 用「同步阻塞」的简单写法，达到了过去要靠复杂异步代码才能达到的高并发——**「写起来像阻塞，跑起来像异步」**。

> 虚拟线程的完整原理（mount/unmount 核心魔法、为什么 JDK 21 才有、「同步阻塞」的两层真相、pinning 坑、和 JDK 8 极致异步的性能对比、函数染色）都已系统整理在专篇：[平台线程 vs 虚拟线程](../concurrency-models/java-thread-and-virtual-thread.md)。这里只点到为止，不重复展开。

**为什么过去大家要用「同步阻塞」？因为它好写。** 下面这段就是经典的 BIO（同步阻塞 IO）服务端——一个连接一个线程，逻辑顺序、清晰直白，但每个线程都死等：

```java
// 同步阻塞（BIO）：accept() 和 read() 都会卡住当前线程死等
ServerSocket server = new ServerSocket(8080);
while (true) {
    Socket socket = server.accept();          // 没连接进来就一直阻塞
    new Thread(() -> {
        var in = socket.getInputStream();
        byte[] buf = new byte[1024];
        int n = in.read(buf);                  // 没数据就一直阻塞，线程干等
        handle(buf, n);
    }).start();                                // 1 万连接就要 1 万个 OS 线程，扛不住
}
```

它好写在「顺序、阻塞、直觉」，致命伤在「一连接一线程」——上万连接就上万个 OS 线程，栈内存和上下文切换直接压垮机器（这就是 [C10K 问题](./06-网络IO模型.md)）。过去为了高并发只能改写成晦涩的 [NIO/Netty 异步](./06-网络IO模型.md)；**而虚拟线程让你把上面这段 BIO 风格的代码几乎原样保留，却能轻松扛住百万连接**——这正是它「游戏规则改变者」的含义。

> 这是巨大的伏笔：虚拟线程的「用户态调度 + 阻塞自动让出」，思想上与 [Go 的 goroutine](../concurrency-models/go-goroutine-csp.md) 高度相似，也与各种 [async/await](../concurrency-models/nodejs-eventloop.md) 殊途同归。第四章 [高并发 HTTP 服务对比](../part4-multilang-compare/01-高并发HTTP服务对比.md) 会把虚拟线程版 Java 和 Go、Rust、Node 放在一起实测对比。
...
<details>
<summary><b>展开：为什么要分这三级？谁决定何时升级？各应对什么场景？</b></summary>

**为什么需要升级（核心动机）：按竞争程度付费。**

早期的 `synchronized` 不管有没有竞争，**一上来就直接用重量级锁**（OS 互斥量 + 挂起线程），所以以前 `synchronized` 一直被骂「慢」。但真实程序里，**绝大多数锁要么根本没人跟它抢、要么竞争很轻**。锁升级（JDK 6 引入）的本质就是「**按竞争程度付费**」——没竞争就走几乎零成本的路径，竞争激烈了才付出昂贵的代价。这也是从 JDK 6 起 `synchronized` 性能不再明显劣于 `ReentrantLock` 的原因。

**谁决定、靠什么决定：JVM 决定，依据是「这把锁此刻的竞争激烈程度」，载体是对象头里的 [Mark Word](#三补-d对象头与-mark-word)。**

锁的状态、持有者线程 ID、指向锁记录的指针都记在 Mark Word 的那几个 bit 里。每次有线程尝试进入 `synchronized`，JVM 就读当前 Mark Word 的状态，判断走哪条路、要不要升级。

**三级各自应对什么场景：**

| 级别 | 应对场景 | 怎么实现 | 成本 |
|------|---------|---------|------|
| **偏向锁** | 全程只有**一个线程**用这把锁（根本没竞争，如早期 `StringBuffer`） | 第一次进入就把线程 ID 用 CAS 写进 Mark Word；**同一线程再来只需比对 ID，一次 CAS 都不做** | 几乎为零 |
| **轻量级锁** | **多线程交替**进入，时间错开、几乎不同时抢 | 每个线程在栈帧建「锁记录」，用 **CAS** 把 Mark Word 换成指向它的指针；抢不到就**自旋**（空转重试），赌对方马上释放 | CAS + 短暂自旋，**不陷内核** |
| **重量级锁** | **真·激烈竞争**：自旋多次仍抢不到、临界区也不短 | 膨胀为 OS **互斥量**，抢不到的线程被**真正挂起阻塞**、让出 CPU，由 OS 在释放时唤醒 | 内核态切换 + 上下文切换，最贵 |

**为什么轻量级锁要先自旋而不是直接挂起？** 因为挂起 / 唤醒线程要陷入内核态、做上下文切换，非常贵。如果临界区很短，自旋几下就拿到锁，比「睡一觉再被叫醒」划算得多。只有自旋也救不回来（说明竞争是真的激烈），才升级到重量级锁去挂起——这时让线程睡觉反而比空转烧 CPU 更省。

**方向：升级基本是单向、不可逆的**（不会从重量级降回轻量级）。一旦升到重量级就回不去了。

</details>

> **一个极其重要的澄清：这三种锁是「谁」决定用哪个？答案是 JVM，不是你的代码。**
>
> 「偏向锁 / 轻量级锁 / 重量级锁」**不是 Java 语言层面的概念，而是 HotSpot 虚拟机的内部实现细节**（JVM 的术语）。它们**只针对 `synchronized` 这一个关键字**，是 JVM 为优化 `synchronized` 而设计的「同一把锁的三种内部形态」。
>
> 关键点在于：**你在 Java 代码里写的永远只是 `synchronized`，从来不会、也无法写「轻量级锁」或「重量级锁」**。你的源码、字节码里都只有 `monitorenter`/`monitorexit` 这一对指令（或方法上的 `ACC_SYNCHRONIZED` 标志）；具体此刻这把锁是偏向、轻量还是重量级，是 **JVM 在运行期根据实际竞争情况自动判断、自动升级**的——对 Java 代码完全透明，你既看不见也控制不了（顶多用 `-XX:-UseBiasedLocking` 这类 JVM 参数粗粒度地开关某个阶段）。
>
> ```java
> // 你能写的只有这个：
> synchronized (obj) {
>     // ... 临界区
> }
> // 至于此刻 obj 上是偏向锁还是重量级锁，由 JVM 看竞争激烈程度自己决定，代码无法指定。
> ```
>
> **对照来看 `ReentrantLock` 就完全不同**：它是 **JUC 类库（纯 Java 代码）** 实现的锁，底层是 [AQS](#四aqsjuc-背后的同步器框架) + CAS + `park/unpark`，**根本没有「偏向/轻量/重量级」这套升级机制**——那套是 `synchronized` 专属的 JVM 优化。所以「锁升级」这个话题只在聊 `synchronized` 时才成立，聊 `ReentrantLock` 时不要套用。
>
> 一句话记牢：**`synchronized` 的三级锁 = JVM 运行期自动调优（你不可控）；`ReentrantLock` = 类库实现的锁（行为由 API 决定，你可控）。**
...
<details>
<summary><b>展开：CAS 不加锁，会不会出现「a 比较→b 比较→a 写→b 写」的交错？</b></summary>

这是理解 CAS 的关键一问。答案：**不会——这正是 CAS 存在的全部意义。** 你担心的交错，恰恰是 CAS 要消灭的那个 bug。

**先看用「普通 if + 赋值」会怎样（这确实会出错）：**

```java
// ❌ 普通非原子写法：compare 和 set 是两条独立指令，中间有缝隙
if (value == expect) {   // compare
    value = newVal;      // set —— 这两步之间可以被插入！
}
```
...
value 初始 = 10
线程 a: if (value == 10) → 成立
线程 b: if (value == 10) → 也成立（a 还没写）
线程 a: value = 11
线程 b: value = 11        → b 基于"已过期的判断"覆盖了 a → 丢更新
```

**但真正的 CAS 不会，因为它的「compare + swap」是一条原子指令，不是两条。**

CAS 不是「先比较再写」两个动作，而是 CPU 的**单条原子指令**（x86 `cmpxchg`），硬件保证「读当前值 → 比较 → 写新值」对所有核来说是**一瞬间、不可分割**地完成，中间没有任何缝隙能插入。物理上靠**缓存一致性协议（MESI）+ 缓存锁 / 总线锁**临时**独占缓存行**，独占期间别的核碰不到这块内存——所以两个核**不可能同时**完成各自的 CAS，硬件把它们串行化了。

真实时序只能是（a、b 被硬件强制串行）：

```
value=10，a 和 b 都读到旧值 10，都想改成 11
T1: 线程 a 的 CAS(10,11) 整体原子执行：当前 10==预期 10 ✅ → 写 11，成功
T2: 线程 b 的 CAS(10,11) 整体原子执行：当前 11≠预期 10 ❌ → 失败，返回 false
    → b 重新 get() 读到 11，重算后再 CAS
```

**关键**：a 的「比较」和「写」之间 b 插不进来，反之亦然。谁先抢到缓存行独占权谁就完整做完，另一个再看时值已变、compare 必然失败 → 失败重试。所以绝不会出现「双方都基于旧值各自成功」的交错。

**一句话**：CAS 的精髓不在「Compare」也不在「Swap」，而在 **「它俩是一个不可分割的整体」**——把你担心的那条缝隙从硬件层面焊死了。这就是它「无锁却线程安全」的根本：不靠锁挡别人，靠「我的读-比-写别人插不进来」的原子性。

</details>

<details>
<summary><b>展开：那两个核能不能"同时"各跑一个原子指令？原子 = 全局只能一个吗？</b></summary>

**关键认知：原子性 ≠ 全局串行。** 「原子」只保证「单个操作自身不可分割」，**并不要求「全世界同一时刻只能有一个原子操作」**。能不能真正同时跑，取决于它们碰的是不是**同一块内存（同一个缓存行）**。

**情况一：操作【不同变量】→ 可以真·并行，两个核同时各跑各的。**

线程 a 在核 0 对变量 X 做 CAS，线程 b 在核 1 对变量 Y 做 CAS——这两个原子指令**完全可以物理上同一时刻执行**，因为各自独占的是不同缓存行，谁也不挡谁。所以「一个进程同一时间只能有一个线程执行原子指令」**是不成立的**，多核并行的价值正在于此。

**情况二：操作【同一变量】→ 不能同时完成，硬件强制串行。**

a、b 都对**同一个变量 X**（同一缓存行）做 CAS 时，两个核**不可能同时完成**——CAS 要独占那个缓存行，而一个缓存行同一时刻只能被一个核独占。硬件（MESI + 缓存锁 / 总线锁）会仲裁所有权：核 0 要改就把该行置为独占 / 修改态、让核 1 的副本失效；核 1 要改就得等核 0 放手再重新获取。这个「争夺缓存行所有权」的过程被硬件**排成一前一后**，于是回到上一节的结论：一个 CAS 成功、另一个看到值已变而失败重试。

> **一句话**：不同内存上的原子操作能真并行；只有**争抢同一块内存（缓存行）**时才被硬件串行化。串行化发生在「内存争用」这一刻，而不是「只要是原子操作就全局排队」。

**⚠️ 反直觉的坑——伪共享（False Sharing）：** 即使 a、b 操作的是**逻辑上不同的两个变量**，但如果它们**恰好落在同一个缓存行里**（如数组相邻元素、对象相邻字段），硬件仍会把它们当「同一块」来争夺所有权，导致本可并行的操作被迫串行、性能暴跌。解法是用**填充（padding）**让热点变量各占一个缓存行——Java 有 `@Contended` 注解专门做这事；`LongAdder` 在高并发下比 `AtomicLong` 快，正是用了「把计数分散到不同缓存行」的思路。
...
<details>
<summary><b>展开：CAS 比的是「旧值」不是「新值」；而「新值」常常是一个函数算出来的</b></summary>

**澄清一：CAS 比较的是「预期旧值 A」，跟你要写进去的「新值 B」毫无关系。**

`CAS(V, A, B)` 三个操作数里，**B（新值）完全不参与比较**，它只是「比较通过后要写入的内容」。比较只发生在 `A`（你开始操作前读到的那个旧值）和「`V` 此刻的真实值」之间：

> 「V 现在是不是还等于我当初读到的 A？是 → 写入 B；不是 → 失败。」

所以**不是**「拿当前值和我要 set 的新值比」，而是「拿当前值和我操作前看到的旧值比，确认这期间没人动过，才敢写」。

**澄清二：写进去的「新值 B」不一定是固定数，常常是一个基于旧值算出来的函数结果。**

分两层看：

- **底层 CAS 指令层面**：`compareAndSet(expect, update)` 的 `update` 必须是一个**已经算好的具体值**——CPU 原子指令内部不可能再「跑一个函数」。
- **使用层面**：但这个具体值，几乎总是「**用一个函数 `f` 基于旧值 A 算出来的**」。看 `incrementAndGet` 里 `next = prev + 1`，新值就是 `f(A) = A + 1`，每次重试 A 变了、算出的 B 也跟着变。

Java 8 起，原子类干脆把「函数」放到了 API 表面，你直接传「如何从旧值算新值」的函数，重试循环由它内部封装：

```java
AtomicInteger ai = new AtomicInteger(10);

// 一元函数：新值 = 旧值 * 2 + 1，内部自动 CAS 自旋重试
ai.updateAndGet(old -> old * 2 + 1);

// 二元函数 + 参数：原子地取最大值
ai.accumulateAndGet(5, (old, x) -> Math.max(old, x));

// 引用类型同理：原子地基于旧对象算出新对象
AtomicReference<String> ref = new AtomicReference<>("a");
ref.updateAndGet(s -> s + "b");
```

`updateAndGet(f)` 内部就是「读旧值 → 用 `f` 算新值 → CAS → 失败就拿最新值重新跑 `f`」。

> ⚠️ **前提：传入的函数必须是无副作用的纯函数**。因为 CAS 失败时它会被**反复调用多次**，若函数里有打印日志、改别的变量等副作用，重试几次就执行几次，会出 bug。

</details>

<details>
<summary><b>展开：CAS 到底比的是哪两个值？是「版本控制」吗？是怎么加锁的？</b></summary>

这里要纠正三个常见误解：

**1. CAS 比的是「值」，不是「版本号」。** CAS 比较的两边是：

> **「内存地址 V 此刻的真实值」** vs **「我手里的预期旧值 A」**

预期值 A 是你之前读出来、暂存在寄存器 / 局部变量里的；V 是那个共享变量在内存里的位置。CAS 的语义就是：「去内存把 V 现在的值读出来，和我给的 A 比——相等（说明我读取后没人动过）就改成新值 B，不等（被别人改过了）就什么都不做、返回失败。」**两边比的都是值。**

**2. CAS 本身不是「版本控制」。** 「版本号」是为修补 CAS 的 [ABA 问题](#考点-4cas-与-aba-问题原子类必问)才**额外**加的、可选的增强——因为只比值时，值从 A→B→A 会被误判成「没变过」，于是在值之外再带一个每次修改就 +1 的戳记，CAS 时连戳记一起比（Java 里的 `AtomicStampedReference`）。所以是「版本控制建立在 CAS 之上」，而不是「CAS 就是版本控制」。

**3. CAS 恰恰是「不加锁」，它是乐观策略。** 它不获取任何锁，而是先假设没人抢、直接尝试改，改的瞬间用一条原子指令检查「我读取后到现在有没有被人动过」——没动过就成功，动过就失败重试。这与传统锁（悲观：先锁门再改）思路相反，是「无锁（lock-free）」的基石。

**那原子性靠什么保证？硬件。** 「比较 + 交换」由一条 CPU 原子指令完成（x86 是 `cmpxchg`，常配 `lock` 前缀）。物理上变量可能在某个核的 L1/L2 cache 里，CAS 执行时该核通过**缓存一致性协议（MESI）+ 缓存锁 / 总线锁**临时**独占这个缓存行**，其它核对这块内存的访问被挡住，从而让「读当前值 → 比较 → 写新值」对所有核看起来一瞬间完成。所以**不是「两个线程的 cache 互相比」，而是某个核拿到缓存行独占权后，用自己手里的预期值 A 去比对那块内存的最新值。**

</details>

<details>
<summary><b>展开：CAS 一定要配自旋吗？两者是固定绑定还是可组合？</b></summary>

**可组合。CAS 和自旋是两件独立的事**，「CAS + 自旋」只是最常见的搭配，不是唯一搭配：

- **CAS** 本身是「**尝试一次**」的原子操作，返回成功 / 失败，**它自己没有循环**。
- **自旋**是套在 CAS 外面的「失败就 `while` 重试」的**外层策略**。

失败后怎么办由你决定，常见组合有：

| 组合 | 做法 | 适用场景 |
|------|------|---------|
| **CAS + 自旋重试** | 失败就立刻重试直到成功（`do{}while(!cas)`） | 竞争不激烈、临界区极短，如 `AtomicInteger` |
| **CAS + 一次性（不自旋）** | 只调一次 `compareAndSet`，失败直接返回 false 交给上层 | 乐观更新失败就报错 / 走别的逻辑 |
| **CAS + 有限自旋后挂起** | 自旋几圈仍失败就 `park` 挂起，不再空转 | `synchronized` 轻量级→重量级升级、[AQS](#四aqsjuc-背后的同步器框架) |
| **CAS + 退避（backoff）** | 失败后等一会儿（甚至随机）再试 | 高竞争下缓解「重试风暴」 |

一句话：**CAS 负责「试一次」，自旋负责「失败后反复试」，两者解耦**，可以只 CAS 一次、可以无限自旋、可以自旋几次后挂起、可以退避再试。
...
<details>
<summary><b>展开：值一样不就行了吗，CAS 目的不就是 set？那「比版本号」到底在比什么？</b></summary>

这是 ABA 最容易卡住的点。先纠正一个理解：

**CAS 的真正目的不是「set 成某个值」，而是「在确认我读取后无人插手的前提下才 set」。** 「set 成功」只是手段，「保证这次更新建立在一个我所认知的、未被破坏的状态之上」才是目的。

**核心区别：值相同 ≠ 状态没变。**

- 只比**值**：回答「现在是不是还等于我当初看到的那个值」。
- 比**版本号**：回答「**从我读取到现在，这个位置有没有被人动过手（哪怕动完又改回了原样）**」。版本号每次修改都 +1、只增不减，所以值绕一圈 A→B→A 回到 A，版本号已从 1 变成 3，CAS 一比就识破「中间有人动过」。

**什么时候「值一样」就够了？** 纯数值场景。比如 `AtomicInteger` 计数，你只关心最终数字对不对，A→B→A 回到 A 那它就是 A，中间过程无所谓——**这种场景 ABA 根本不是问题，不需要版本号。**

**什么时候「值一样」会出事？** 当变量的值只是某个复杂状态的**引用 / 指针 / 标记**，值相同不代表背后那个东西没被换过。经典例子是**无锁栈出栈**：

```
初始：top → A → B → C
```

1. 线程 1 要出栈，读到 `top = A`，算出「弹出后新栈顶 = A.next = B」，正要 `CAS(top, 期望A, 新值B)` 时被挂起。
2. 线程 2 趁机：弹出 A、弹出 B、又把 A push 回来。栈变成 `top → A → C`（B 已被移走、内存可能已回收）。
3. 线程 1 醒来执行 `CAS(top, 期望A, B)`：发现 `top` 的值**还是 A**（值相同！）→ **CAS 成功**，把 top 设成了 B。
4. **灾难**：B 早已不在栈里（甚至已被回收），top 现在指向一个失效节点，链表结构彻底坏掉。

问题就出在：CAS 只看到「top 的值还是 A」，就误以为「这期间没人动过、我之前算出的 B 仍有效」——**值相同骗过了 CAS**。若带版本号，线程 2 的两 pop 一 push 让版本从 1 涨到 4，线程 1 的 CAS 因版本不匹配而失败重试，重新读最新状态、重新计算，灾难即可避免。

**一句话**：版本号回答的是「值」回答不了的那个问题——「这中间到底有没有被人动过手」。纯数值看值就够，引用 / 结构型必须靠只增不减的版本号兜底。

</details>

<details>
<summary><b>展开：`compareAndSet` 是真方法吗？为什么参数里没有「变量名」也没有「版本号」？</b></summary>

`compareAndSet` 是 `java.util.concurrent.atomic` 包里**真实存在**的方法。它确实只有两个参数，原因如下：

**问题一：为什么没有「要改的变量」（如 abc）？因为变量就是「调用方法的那个对象」，通过 `this` 隐式传入。**

`compareAndSet` 是**实例方法**，必须通过一个原子对象去调用，点号前的接收者就指明了「改谁」：

```java
AtomicInteger abc = new AtomicInteger(10);   // abc 本身就是那个"被改的变量"
abc.compareAndSet(10, 11);                    // 在 abc 上做 CAS：当前==10 就改成 11
//  ↑接收者=变量      ↑预期旧值 ↑新值
```

就像 `list.add(x)` 不会写成 `add(list, x)`——`list` 这个接收者已经指明往哪加。CAS 同理，**变量在点号前面，不在括号里**。它内部维护一个 `volatile int value` 字段，改的就是它。

**问题二：为什么没有「版本号」？因为普通原子类本来就不带版本号（所以才有 ABA 问题）。版本号是专门的类才有的。**

```java
// 普通原子类：只比值，签名 (旧值, 新值)，无版本号 → 有 ABA 风险
AtomicReference<String> r1 = new AtomicReference<>("A");
r1.compareAndSet("A", "B");

// 带版本号(戳)的 AtomicStampedReference：签名多了"预期旧版本 + 新版本"
AtomicStampedReference<String> r2 = new AtomicStampedReference<>("A", 0); // 初值A, 初版0
int[] holder = new int[1];
String cur = r2.get(holder);                  // 同时取出当前值与当前版本
int curStamp = holder[0];
r2.compareAndSet("A", "B", curStamp, curStamp + 1);
//               ↑旧值 ↑新值 ↑预期旧版本   ↑新版本
```

`AtomicStampedReference.compareAndSet` 的真实签名有**四个参数**，正是你直觉里那个「带版本号的版本」：

```java
boolean compareAndSet(V expectedReference, V newReference,
                      int expectedStamp,   int newStamp)
// 语义：当且仅当 当前值==预期旧值 且 当前版本==预期旧版本，
//       才同时把值改成新值、版本改成新版本（两个都得匹配，才能识破 A→B→A）
```

**对照你设想的 `abc.compareAndSet(旧值, 版本, 新值)`：**

| 你设想的部分 | 真实情况 |
|---|---|
| `abc`（要改的变量） | ✅ 就是点号前的**调用接收者**（`this`），不进参数列表 |
| 预期旧值 | ✅ 第一个参数 |
| 新值 | ✅ 最后一个参数 |
| 版本号 | ⚠️ 普通原子类**没有**；只有 `AtomicStampedReference` 才有，且是「预期旧版本 + 新版本」两个参数 |

你的直觉几乎全对，只差两点：① 变量靠对象接收者表达、不进参数；② 版本号不是所有 CAS 都有，是带戳的专用类才有。
...
<details>
<summary><b>展开：ThreadLocal 到底存的是什么？是栈帧数据吗？存在哪？</b></summary>

要分清「**存什么**」和「**存在哪**」两件事。

**存什么（内容）：你自己 `set` 进去的那个对象，即用户自定义数据——不是栈帧数据。** 可以是任意对象：一个 `SimpleDateFormat`、一个数据库 `Connection`、一个用户上下文 `UserContext`…… ThreadLocal 不关心你存什么，只负责「让每个线程各存各的、互不干扰」。

**存在哪（位置）：数据不存在 ThreadLocal 对象里，而是存在「当前线程对象」里。** 结构是这样的：

```
Thread 对象（堆上）
  └─ ThreadLocal.ThreadLocalMap threadLocals   ← 每个线程自己一份
        ├─ key = ThreadLocal 实例A  → value = 你 set 进去的数据A
        └─ key = ThreadLocal 实例B  → value = 你 set 进去的数据B
```

- `threadLocal.set(v)`：拿到**当前线程** → 找到它身上的 `ThreadLocalMap` → 以「这个 threadLocal 对象」为 key 存入 v。
- `threadLocal.get()`：拿到**当前线程** → 它的 map → 用「这个 threadLocal」当 key 取出 v。

**为什么天生线程隔离？** 因为线程 A、B 各有各的 `ThreadLocalMap`，哪怕用同一个 ThreadLocal 变量当 key，取出来的也是各自线程里存的那份，物理上就分开了，根本不会串。

**为什么不是栈帧数据？** 栈帧存方法的局部变量、操作数栈，随方法调用入栈、返回出栈，生命周期跟着**方法**。而 ThreadLocal 的数据存在**堆上的 ThreadLocalMap**（Thread、Map、值都在堆中），生命周期跟着**线程**——只要线程活着、你没 `remove()`，跨多个方法调用都能取到。这正是它的用途：在**同一线程的不同方法间隐式传递数据**，不必层层传参。

> **一个常见误解**：以为「值存在 ThreadLocal 里、一个 ThreadLocal 只能存一个值」。实际上一个 ThreadLocal 在 N 个线程里就有 N 份值（各存在各线程的 map 里）；一个线程也能持有多个 ThreadLocal。**真正的存储中心是「线程」，ThreadLocal 只是一把「在某线程的 map 里定位我那份值」的钥匙（key）。**

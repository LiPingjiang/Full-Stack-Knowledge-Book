...
实际的 Postings List 比这个例子复杂：除了文档号，还会存**词频**（TF，用于 [BM25 打分](#51-从-tf-idf-到-bm25)）和**词的位置**（用于短语查询 `match_phrase` 和高亮）。它在磁盘上的存储格式、压缩方式和加速结构，详见 [6.12 底层存储引擎](#612-底层存储引擎lucene-到底长什么样)。
...
**③ 查询是分散-聚合（Scatter-Gather）**。搜索请求到达任意节点（该节点成为本次的协调节点），它把请求**并行**发给 P0/P1/P2 各一份（或者它们的副本，走轮询负载均衡），每个分片在本地独立算出 Top N，返回给协调节点做全局归并排序。这就是 ES 能横向扩展的根本原因，也是[深分页](#69-深分页的三种方案)慢的根本原因。
...
**ES 用的不是二阶段提交（2PC）**——这是个高频误解，值得说清楚。

```
二阶段提交（2PC）：
  阶段一 Prepare：协调者问所有参与者"能提交吗？" → 各方预留资源并回复
  阶段二 Commit ：全票通过才发 Commit，任一否决则全体 Rollback
  特点：有全局事务、可回滚、阻塞式

ES 的主备同步复制（Primary-Backup Replication）：
  只有一个阶段：主分片【先自己写成功】，再并行转发给所有副本
  副本【没有否决权】，它们不能说"我不行，大家一起回滚"
  特点：无全局事务、【不可回滚】、写入即生效
```

**关键差异：ES 的主分片是"先斩后奏"。** 它写完自己就已经既成事实了，转发给副本只是让副本追上，而不是征求同意。所以 ES **没有跨文档的原子性**，也**不支持回滚**——这正是 [6.12](#612-底层存储引擎lucene-到底长什么样) 说"ES 不适合做主数据库"的原因之一。

那么副本写失败了怎么办？**ES 的选择是"踢掉副本，而不是回滚主分片"**：
...
主分片写成功 → 转发给 R1、R2
                 ...
                 └─ R2 失败（节点挂了 / 超时）
                        ↓
       主分片向 Master 上报："R2 有问题，把它踢出 in-sync 集合"
                        ↓
       Master 更新 Cluster State，R2 被标记为失效
                        ↓
       只要 R1 成功，本次写入仍然【返回客户端成功】
                        ↓
       后续 Master 会在别处重建一个新的 R2，从主分片全量/增量恢复
```

这里的 **in-sync 集合（同步副本集合）**是核心概念——它记录"当前哪些副本的数据是跟得上主分片的"。只有 in-sync 集合里的副本才有资格在主分片挂掉时被提升为新主分片，这保证了不会把一个落后的副本提上来导致数据丢失。

**写入成功的判定标准由 `wait_for_active_shards` 控制**：

```
默认值 1  = 只要【主分片】写成功就返回 OK（不等副本）
设为 all  = 主分片 + 所有副本都写成功才返回
设为 2    = 至少 2 个分片副本（含主）可用才开始写

⚠️ 注意它是【写入前的准入检查】，不是"等多少个写完"
   即使设为 all，它检查的也是"活跃分片数是否达标"，
   真正的写入仍然是主分片成功即算数据落定
```

> **权衡**：`wait_for_active_shards=1`（默认）延迟最低，但主分片刚写完就宕机、副本还没收到时会丢数据；设为 `all` 更安全但延迟高、且任一副本不可用就会写入阻塞。**生产上绝大多数场景用默认值 1**，靠 Translog（见 [6.5](#65-segment-与写入性能)）和副本机制兜底，而不是靠同步等待所有副本。

**seq_no 与 primary_term——保证副本不错乱的两个序号**：

```
_primary_term ：主分片的"任期号"。每次主分片易主（故障切换）就 +1
                → 类比 Raft 的 term，用来识别"过期的主分片"
_seq_no       ：该分片上每次写操作的递增序号
                → 用来判断操作的先后顺序

作用：老主分片"复活"后带着旧 primary_term 来写数据，
      副本一看任期过期，直接拒绝 —— 防止脑裂导致的数据错乱
```

这两个序号还支撑了**乐观并发控制**——这是 ES 处理并发更新的方式：

```json
// 读取时拿到 _seq_no 和 _primary_term
GET /orders/_doc/1001
// → { "_seq_no": 42, "_primary_term": 3, "_source": {...} }

// 更新时带上，若期间被别人改过则 seq_no 已变，本次请求失败（409 冲突）
PUT /orders/_doc/1001?if_seq_no=42&if_primary_term=3
{ "status": "PAID" }
```

> **这是 ES 版的 CAS**：和 MySQL 用 `version` 字段做乐观锁是同一个思路。**ES 没有悲观锁，也没有事务**，并发更新只能靠这套机制在应用层重试。如果两个请求并发更新同一文档，后到的那个会收到 409，需要重新读取再重试——这也解释了为什么高频更新的场景不适合放在 ES（见 [6.5](#65-segment-与写入性能) 的更新代价分析）。
...
ES 写入后**不是立即可搜**，而是有约 1 秒的延迟（`refresh_interval`）。写入流程：数据先进 in-memory buffer → 每秒 refresh 到一个新的 segment（[Lucene](#612-底层存储引擎lucene-到底长什么样) 可搜索的最小单元）→ 后台 merge 合并小 segment。这就是"近实时"的由来。
...
> **调优前必读**：BM25 参数调优的收益，通常远小于「改进分词」（见 [6.1](#61-分词器深入ik-的工程用法)）和「加 Rerank」（见 [6.11](#611-怎么给-es-加-rerank)）。**先把分词和同义词做好，再考虑动 k1/b**。而且改了 `similarity` 需要重建索引，成本不低。
...
>
> **为什么老文档不会自动更新**：倒排索引是在**写入时**根据当时的分词结果构建的，且 Lucene 的 segment 不可变（见 [6.5](#65-segment-与写入性能)）。词典只影响"分词这个动作"，不会回溯改写已经落盘的索引。所以「加了词还是搜不到」十有八九不是词典没生效，而是**忘了重建历史数据**——先用 `_analyze` 确认词典本身是好的，再去查数据。
...
> **实践中最常用的其实是第四种做法：滚动索引 + 别名**。与其纠结分片数改不了，不如一开始就按时间滚动（`logs-2026.08.01`、`logs-2026.08.02`…），用别名对外提供统一视图。这样新索引可以随时用新的分片数，老索引到期直接删除，**完全绕开了"分片数不可变"这个问题**——这也是时序场景的标准解法。`_shrink` 则常用在 ILM 的 Warm 阶段（见 [6.6](#66-冷热分离与索引生命周期ilm)）：数据不再写入后，把分片数降下来省资源。
...
**默认所有非 `text` 字段的 Doc Values 都是开启的**（`text` 字段因为分词后基数太大，默认关闭，要排序聚合得用 `fielddata` 或加 `.keyword` 子字段）。

> **fielddata 是什么，为什么危险**：Doc Values 是**写入时**就建好的磁盘列存；而 `fielddata` 是**查询时**临时把 `text` 字段的整个倒排索引反转成正排结构，并**全量常驻 JVM 堆内存**。一个大文本字段开 fielddata 极易直接打爆堆，所以 ES 默认禁用它，且用熔断器（见 [6.7](#67-集群调优要点)）限制其内存占比。**生产环境的正确做法永远是加 `.keyword` 子字段，而不是开 fielddata。**

Doc Values 默认开启这件事意味着——**一个你从来不聚合、不排序的 `keyword` 字段，ES 仍然在默默为它写一份列存，白白占磁盘和写入 IO**。
...
>
> **代价与坑**：关掉之后，该字段**无法排序、无法聚合、无法用于 `script` 取值、无法作为 `sort` 的 tie-breaker**（这点尤其容易踩——[search_after 深分页](#69-深分页的三种方案)要求排序字段唯一，如果你把兜底字段的 doc_values 关了就用不了）。而且**改这个属性需要重建索引**，属于典型的"设计时就要想清楚"的决策。
...
**`should` 的陷阱——它不是过滤器**。这是最常见的 bool 查询误用，会直接造成功能性 bug：

```
should 的语义随上下文变化：

  bool 里【有】 must 或 filter  → should 退化为"可选加分项"
                                  不满足也照样命中，完全不起过滤作用
  bool 里【只有】should        → 至少要满足 1 个
                                  （minimum_should_match 默认为 1）
```

```json
// ✗ 典型 bug：想做品牌筛选，却把它放进了 should
{
  "bool": {
    "must":   [ { "match": { "title": "羽绒服" } } ],
    "should": [ { "term":  { "brand": "优衣库" } } ]     // 形同虚设！
  }
}
// 结果：返回【所有品牌】的羽绒服，优衣库的只是排前面
//       用户勾选的品牌筛选完全失效
```

```json
// ✓ 正确：用户勾选的筛选条件是硬性条件，放 filter
{
  "bool": {
    "must":   [ { "match": { "title": "羽绒服" } } ],
    "filter": [
...
`should` 真正的用武之地是**多字段软匹配**——命中了加分、没命中也不排除：
...
```json
{
  "bool": {
    "must":   [ { "match": { "title": "羽绒服" } } ],
    "should": [
      { "match": { "description": "羽绒服" } },        // 描述也命中 → 加分
      { "term":  { "is_flagship": true } }             // 旗舰店 → 加分
    ]
  }
}
```
...
| 需求 | 用哪个 | 是否打分 | 可缓存 |
|------|-------|---------|--------|
| 必须满足，且影响排序（全文匹配） | `must` | ✓ | ✗ |
| 必须满足，不影响排序（状态/权限/筛选项） | **`filter`** | ✗ | ✓ |
| 必须不满足（排除） | `must_not` | ✗ | ✓ |
| 不必满足，满足了加分 | `should` | ✓ | ✗ |

> **一句话记忆**：`must` = 必须且打分，`filter` = 必须但不打分，`should` = 不必须但加分，`must_not` = 必须没有。**用户在界面上勾选的筛选条件，99% 的情况都该进 `filter`，而不是 `should`。**

### 6.4 相关性加权——把业务信号融进排序

纯 BM25 只看文本匹配度，但真实业务里排序还要考虑**销量、评分、时效性、是否旗舰店**这类**业务质量信号**。这类需求的典型表述是「**相关性为主，销量高的加权**」——注意它要的是**一个融合分数**，不是两个排序字段。

**首先要避免的错误：用 `sort` 实现加权**。

```json
// ✗ 一旦指定 sort，_score 就完全不参与排序（ES 还会跳过打分计算）
{
  "query": { "match": { "title": "羽绒服" } },
  "sort":  [ { "sales_30d": "desc" } ]
}
// 结果退化成"按销量排的所有羽绒服"
// 标题里只顺带提一句羽绒服的爆款睡衣，会排在专业羽绒服前面
```

**正解一：`function_score`——通用但较重**
...
```json
{
  "query": {
    "function_score": {
      ...
      "functions": [
        {
          "field_value_factor": {
            "field":    "sales_30d",
            "modifier": "log1p",        // 关键：压缩量级差异
            "factor":   0.1,
            "missing":  0               // 字段缺失时的默认值，防止报错
          }
        },
        {
          "gauss": {                     // 时效性衰减：越新分越高
      ...
      "score_mode": "sum",              // 多个 function 之间如何合并
      "boost_mode": "multiply"          // function 结果与 _score 如何合并
    }
  }
}
```
...
```
modifier: log1p  ← 必须做量级压缩
  销量是长尾分布（爆款 10 万，普通款 50）
  直接相乘会让销量彻底压倒相关性 —— 又变回按销量排序了
  取对数把 10万:50 = 2000倍 压缩到 ln(100001):ln(51) ≈ 2.9倍
  【这和 BM25 用饱和函数处理词频是同一个思路，见 5.2】

boost_mode: multiply（推荐）
  最终分 = _score × 加权因子   → 相关性是基础盘，业务信号只做倍率调整
boost_mode: sum（危险）
  最终分 = _score + 加权因子   → 加权因子可能直接盖过 _score

factor: 0.1
  纯业务参数，必须靠 A/B 测试调，没有标准答案
```

**正解二：`rank_feature`——ES 7.x+ 的更优解**

如果加权信号是固定的几个数值字段（销量、评分、PageRank 值），用 `rank_feature` 字段类型性能更好：

```json
// 1. Mapping 里声明为 rank_feature 类型
{
  "properties": {
    "sales_30d":  { "type": "rank_feature" },
    "shop_score": { "type": "rank_feature" },
    "return_rate":{ "type": "rank_feature", "positive_score_impact": false }
                    // 退货率越高分越低 → 声明为负向信号
  }
}

// 2. 查询时作为 should 子句参与打分
{
  "bool": {
    "must":   [ { "match": { "title": "羽绒服" } } ],
    "filter": [ { "term": { "is_deleted": false } } ],
    "should": [
      { "rank_feature": { "field": "sales_30d",  "log": { "scaling_factor": 10 } } },
      { "rank_feature": { "field": "shop_score", "saturation": { "pivot": 8 } } }
    ]
  }
}
```

**为什么 `rank_feature` 更快**：`function_score` 要**对每个命中文档逐个执行函数计算**；而 `rank_feature` 把数值编码进倒排索引，**在倒排层面就参与打分**，还支持提前终止（能跳过不可能进入 Top N 的文档）。代价是只支持正浮点数、且必须提前在 mapping 里声明。

| | `function_score` | `rank_feature` |
|---|---|---|
| 灵活性 | 高（支持脚本、随机、地理衰减） | 低（只支持数值字段的几种曲线） |
| 性能 | 逐文档计算，较慢 | 倒排层参与，快，支持提前终止 |
| 字段要求 | 任意数值字段 | 必须声明为 `rank_feature` 类型 |
| 适用 | 复杂/动态的打分逻辑、地理位置衰减 | **固定的静态质量分（销量/评分/权重）** |

> **选型建议**：静态质量分优先用 `rank_feature`；需要地理位置衰减、时间衰减或自定义脚本时用 `function_score`。两者可以混用。**但更重要的是——如果排序逻辑复杂到要写一堆函数，说明你真正需要的是 [Rerank](#611-怎么给-es-加-rerank) 或 Learning to Rank，而不是在 DSL 里堆参数。**

### 6.5 Segment 与写入性能

理解 segment 是理解 ES 写入性能的关键。回顾 [3.3](#33-近实时near-real-time原理) 的写入流程，补充完整（Segment 内部的文件结构和 FST 词典见 [6.12](#612-底层存储引擎lucene-到底长什么样)）：

```
写入完整链路：

  ① 文档写入 In-memory Buffer + 同时追加到 Translog（预写日志，保证不丢）
...
**merge 是自动的，不需要手动触发**——这一点容易误解。Lucene 后台线程持续运行 `TieredMergePolicy`（分层合并策略），每次 refresh 产生新段后都会检查是否该合并：

```
默认策略的核心规则：
  · 把大小相近的 Segment 归为一层（tier）
  · 一层攒够 10 个（segments_per_tier）就合并成一个更大的
  · 单次最多合并 10 个段（max_merge_at_once）
  · 超过 5GB 的段不再参与常规合并（max_merged_segment）
  · 删除文档占比超过 33% 的段优先合并（deletes_pct_allowed）

merge 线程数默认 max(1, min(4, 核数/2))，合并完成后【删除旧段文件】
→ 磁盘空间在这一刻才真正释放
```

所以「删除文档占着磁盘」通常不是 merge 没跑，而是**写入速度超过了 merge 的清理速度**。频繁更新的索引会持续产生僵尸文档，merge 永远追不上，表现为磁盘持续增长、查询持续变慢。这种情况下真正的解法是**减少写入量**（比如把高频变更的字段移出 ES），而不是手动强推合并。

如果只是想让清理更积极，可以调低删除文档的容忍比例：

```json
PUT /orders/_settings
{
  "index.merge.policy.deletes_pct_allowed": 15    // 默认 33，调低使 merge 更早介入
}
```

**自动 merge 与 `_forcemerge` 的区别**：

| | 自动 merge | `_forcemerge` |
|---|---|---|
| 触发方式 | 后台持续自动运行 | 手动调 API |
| 目标 | 维持段数在合理范围 | 强行合并到指定段数（通常 1） |
| 5GB 上限 | 遵守 | **无视**，可产生超大段 |
| 资源占用 | 受限流控制 | 吃满 IO 和 CPU |
| 适用对象 | 所有索引 | **只读索引** |

> **`_forcemerge` 的使用禁忌**：只对**不再写入**的只读索引执行。对活跃写入的索引强制合并会产生巨大的段，后续 merge 极其昂贵，而且合并期间 IO 打满会影响线上查询。这是运维事故的常见来源。
>
> 具体的恶性循环是这样的：forcemerge 合出一个 50GB 的超大段 → 突破了 `max_merged_segment` 上限 → 此后该段再产生删除文档，常规 merge 也不愿意碰它 → 删除文档越积越多永远清理不掉 → 只能再 forcemerge 一次，代价越来越大。**正确用法是在索引停止写入后（如按天滚动的日志索引的"昨天"那个），于低峰期执行一次**，这也是 ILM Warm 阶段的标配动作（见 [6.6](#66-冷热分离与索引生命周期ilm)）。
...
堆内存设置铁律：
  ...
  ② 不超过 31 GB
     → 超过 32GB 会失去「压缩指针（Compressed OOPs）」
       （JVM 对象按 8 字节对齐，故 4 字节偏移量可寻址 2^32×8 = 32GB；
         超过后只能退回 8 字节裸指针，指针占用翻倍）
...
GET /docs/_search
{
  "knn": {
    ...
    "k": 10,
    "num_candidates": 100,          // 对应 HNSW 的 efSearch，越大召回越高越慢
    "filter": [                     // 过滤前置，避免召回塌陷
      { "term": { "tenant_id": "t_42" } }
    ]
  }
...
一个 Shard = 一个 Lucene Index = 多个 Segment + 一个 Translog
        ...
        │ .nvd / .nvm    │ Norms：字段长度归一化因子           │ ← BM25 惩罚长文档用
        │ .liv           │ 存活文档位图（标记删除）                  │
        └────────────────┴───────────────────────────────────────┘
```

> **Translog 是什么**：ES 的**事务日志**（Transaction Log），作用和 MySQL 的 **redo log** 完全一致——都是 **WAL（Write-Ahead Log，预写日志）**思路：数据还在内存缓冲区、尚未落盘成 Segment 时，先把操作顺序追加到日志里，进程崩溃后重放日志即可恢复。**这正是 refresh（1 秒）和 flush（30 分钟）能分开的原因——有 Translog 兜底，不必每次都 fsync。**

> **Norms 是什么**：索引时预先算好并存下来的**字段长度归一化因子**，供 BM25 公式里的 $\frac{|D|}{\text{avgdl}}$ 直接取用（见 [5.2](#52-k1-与-b-两个调优参数)）——不然每次打分都要重新数一遍文档有多长。为省空间它只用 **1 字节**有损存储，所以长度归一化本身就是粗粒度的。不需要相关性评分的字段可设 `"norms": false` 省磁盘。
...
**FST 到底是什么**——名字里三个词都要拆开看，而且"共享前后缀的字典"和"有限状态转换器"其实是**同一个东西的两种说法**：一个是**结果**（它长得像压缩字典），一个是**原理**（它本质是台状态机）。

先看它的三层演进，从最熟悉的 Trie 开始：

```
第一步：Trie（前缀树）—— 每条边是一个字符，共享前缀
────────────────────────────────────────────────
  存 [cat, cats, dog, dogs]
  ...
  问题：后缀 "s" 存了两次，(4) 和 (8) 结构完全相同却是两个节点

第二步：FSA（有限状态自动机）—— 合并等价状态，前后缀都共享
────────────────────────────────────────────────
        (0)
       c/   \d
      (1)    (5)
  ...
  这就是"共享后缀"。节点数从 9 降到 6。
  但它只能回答"这个词在不在"，回答不了"这个词对应什么值"

第三步：FST（有限状态转换器）—— 边上除了字符，再挂一个输出值
────────────────────────────────────────────────
  Transducer = "转换器"，意思是它做的是【映射】而不只是【判断】
  输入一个词 → 输出一个值（Lucene 里这个值是倒排表的磁盘偏移量）

        (0)
    c/5    \d/12          ← 边的标签变成「字符 / 输出值」
     (1)     (5)
    a/0|      |o/0
     (2)     (6)
    t/0|      |g/0
  ...
  查 "cat"：走 c→a→t，把沿途输出相加 5+0+0 = 5   → 偏移量 5
  查 "cats"：走 c→a→t→s，累加 5+0+0+3 = 8        → 偏移量 8
  查 "dog"：走 d→o→g，累加 12+0+0 = 12           → 偏移量 12
```

**合并后为什么不会认错词**——这是理解 FST 最容易卡住的地方。用一个刁钻的例子：`battle` 和 `but` 都含字母 `t`，图里的某个 `t` 到底属于谁？

```
关键：状态只记录"接下来能怎么走"，不记录"我从哪来"。

  b-a-t-t-l-e     和     b-u-t
      ↑ ↑                    ↑
   这两个 t                 这个 t

  它们能不能合并，只取决于【走到那一步之后，后续可能性是否相同】：

    battle 的第 1 个 t：后面必须接 "tle"      → 后续 = {t}
    battle 的第 2 个 t：后面必须接 "le"       → 后续 = {l}
    but    的那个 t   ：到此结束，是终止态    → 后续 = {} 且终止

  三者后续行为完全不同 → 三个独立状态，谁也不会合并
```

真正会被合并的是**后缀完全相同、且后续行为一致**的部分：
...
存 [battle, cattle]
  ...
  而 "but" 里的 t 后面什么都不接，行为不同 → 绝不会并到这里来
```

所以判断依据不是"字符长得一样"，而是**从这个状态出发，能走的路径集合是否完全相同**（学术上叫"右语言相同"）。`battle` 里的两个 `t` 尽管字符相同、位置相邻，也是两个不同的状态。

**那走到汇合点后，怎么知道自己匹配的是 battle 还是 cattle？**——靠**路径**，不靠节点。你从 `b-a-t` 来还是从 `c-a-t` 来，节点 (7) 本身并不知道，但**你走过的边序列就是答案**。这也意味着 FST 只支持从头正向查找，无法从中间节点反推有哪些词经过它——但这不影响使用，因为查词本来就从第一个字符开始。

**输出值如何保证唯一**——共享的边被多个词复用，靠**沿路累加**区分：

```
  battle = b/5 + a/0 + t/0 + [t/0 + l/0 + e/0]  = 5
  cattle = c/9 + a/0 + t/0 + [t/0 + l/0 + e/0]  = 9
                              └─ 共享的这段贡献相同 ─┘

  起点输出不同（5 vs 9）→ 即使共用后半段，结果依然唯一
```

这就是为什么输出值要设计成"沿路累加"而不是"终点直接存"——**只有累加，才能让同一条共享边对不同的词产生不同的最终结果**。

现在可以精确回答你的三个问题：

**① 每个点是一个字符还是一个词？**——**边（箭头）上是字符，点（节点）是状态**。一个完整的词是**一条从起点到终止状态的路径**，不是一个节点。所以 3000 万个词并不需要 3000 万个节点，公共前后缀被大量复用。

> 严格说 Lucene 的 FST 边上存的是**字节（byte）**而非字符，中文 UTF-8 一个字占 3 字节，就是 3 条边。

**② 字典里的"状态"是什么？**——**状态就是"我已经匹配到了哪个前缀"这件事本身**。

```
状态 (0)：还没输入任何字符
状态 (1)：已经匹配了 "c"
状态 (2)：已经匹配了 "ca"
状态 (3)：已经匹配了 "cat" —— 且这是个合法的词（终止状态）
```

关键洞察是：**状态不记录"我走过哪条路"，只记录"接下来还能怎么走"**。所以 `cat` 和 `dog` 才能在图里汇合到同一个节点 (3)——因为走到那一步之后，两者后面能接的东西完全一样（要么结束，要么接 `s`）。**行为等价的状态就可以合并，这正是 FST 比 Trie 省内存的根本原因。**

**③ 状态怎么变化？**——靠**读入一个字符触发一次转移**，这就是"有限状态机"的标准工作方式：

```
查询 "cats"：
  当前状态 (0)，读入 'c' → 转移到 (1)，累加输出 5
  当前状态 (1)，读入 'a' → 转移到 (2)，累加输出 0
  当前状态 (2)，读入 't' → 转移到 (3)，累加输出 0
  当前状态 (3)，读入 's' → 转移到 (4)，累加输出 3
  输入耗尽，且 (4) 是终止状态 → 命中，输出 = 5+0+0+3 = 8

查询 "car"：
  (0) --c--> (1) --a--> (2)，读入 'r' → 状态 (2) 没有 'r' 这条出边
  → 立即判定不存在，不需要走完
```

"有限"指的是状态数量有限且固定（构建完就不变了）；"转换器"指的是它在识别输入的同时**产出输出值**——普通状态机只回答"接受/拒绝"，转换器还能给你一个结果。

**图里的星号（终止状态）是什么意思**——注意一个容易误解的点：**"走到路径末端"不等于"这是一个词"，"是一个词"也不等于"不能再往下走"。**

```
词典 = [cat, cats]

      (3)*   ← 终止状态：到这里 "cat" 是一个完整的词
       |s    ← 但它【还有出边】，可以继续走
      (4)*   ← 也是终止状态："cats" 也是一个完整的词
```

节点 (3) 既是 `cat` 的终点，又是通往 `cats` 的中转站——**两个身份并不冲突**。所以判定一次查询是否命中，需要**两个条件同时成立**：

```
  ① 输入的每个字符都能找到对应的出边（路径走得通）
  ② 输入耗尽时，所停留的节点是【终止状态】

词典 = [cat, cats] 时的四种情况：

  查 "cat"    → 走到 (3)，输入耗尽，(3) 是终止态       → 命中 ✓
  查 "cats"   → 走到 (4)，输入耗尽，(4) 是终止态       → 命中 ✓
  查 "ca"     → 走到 (2)，输入耗尽，(2) 不是终止态     → 不存在 ✗
  查 "car"    → 在 (2) 找不到 'r' 出边                 → 不存在 ✗
```

第三种最说明问题：**`ca` 是 `cat` 的合法前缀，路径完全走得通，但它不是词**。如果没有终止标记，FST 就会把所有前缀都误判成词。反过来，如果词典里只有 `cats` 没有 `cat`，那么节点 (3) 就不带星号，查 `cat` 同样判定不存在。

**这个设计正是 ES 前缀查询和自动补全的基础**：

```
用户输入 "分布"，要给出补全建议

  ① 在 FST 上走 分→布，到达某个中间节点
  ② 从该节点出发，遍历下游所有可达的【终止状态】
  ③ 收集路径 → [分布式, 分布式锁, 分布式事务, 分布图 ...]
```

**哈希表做不到这件事**——哈希把 key 打散了，`分布式` 和 `分布式锁` 在哈希表里毫无关联。而 FST 保留了字典序结构，**"共享前缀"于是从"省内存的手段"升级为"支持前缀检索的能力"**，这是一份收益的两种兑现方式。

模糊查询（`fuzzy`、`wildcard`、正则）同理，本质是**在这张图上做带容错的遍历**——允许跳过、替换或插入若干条边，看能否走到终止状态。编辑距离越大，允许的容错步数越多，遍历代价也越高，这就是 `fuzziness` 参数调大后查询显著变慢的原因。

```
FST 在 Lucene 中的三个特性：
  ① 内存占用极小——数十 MB 内存可容纳数千万词条
  ② 常驻堆内存（.tip 文件），命中后拿偏移量再去磁盘读 .tim/.doc
  ③ 天然有序 → 支持前缀查询、范围查询、模糊查询（wildcard/正则本质是在图上做带容错的遍历）
```

这就是 ES 能在亿级文档上做到毫秒级检索的核心——**内存里放 FST 快速定位偏移量，磁盘上放倒排表按需读取**。

Postings List 内部还有两层优化。第一层是**[跳表（Skip List）](./A1-核心数据结构原理.md#一跳表skip-list给链表加目录)**，用于快速求交集——搜"Java 并发"要对两个文档列表求交，跳表让它不必逐个比对。

第二层是**压缩**，两种手段配合使用：

```
Frame of Reference（FOR，差值编码）——适合稠密的文档号
  原始 doc id：[100, 102, 105, 109]
    ↓ 存相邻差值（文档号是递增的，差值很小）
  基值 100 + 差值 [2, 3, 4]
    ↓ 按块内最大差值所需的最小比特数打包
  最大差值 4 → 3 bit 就够 → 原本 4×32 bit 压到 3×3 bit

Roaring Bitmap（分桶位图）——适合大跨度、稀疏分布
  把 doc id 按高 16 位分桶，每个桶内自动选存储方式：
    桶内元素少（≤ 4096）→ 用排序数组，省空间
    桶内元素多（> 4096）→ 用 bitmap，位运算快
  → 兼顾压缩率和交/并集运算速度，[filter 缓存](#63-filter-vs-query-与缓存机制)的 bitset 用的就是它
```
...
> 上表是从**使用者视角**看两者的分工。如果想从**存储引擎结构**层面理解这些差异的根源——为什么 ES 无事务、为什么更新代价高、为什么必然是近实时——见 [6.12 底层存储引擎](#612-底层存储引擎lucene-到底长什么样)中 B+ 树与 Lucene Segment 的对比。
...
MySQL FULLTEXT 索引功能有限（中文分词差、不支持复杂查询、不支持分布式）。ES 基于 [Lucene](#612-底层存储引擎lucene-到底长什么样) 的倒排索引，原生支持分词、相关性评分、分布式分片并行查询、丰富的查询 DSL，是专门为搜索场景设计的。

**追问「倒排索引那么大，怎么快速定位到某个词」**：这是区分"背过定义"和"真懂原理"的关键一问。答案是 **FST 词典索引**——倒排索引本身也需要索引，Lucene 把词典压缩成 FST 常驻堆内存，定位后再去磁盘读倒排表（详见 [6.12](#612-底层存储引擎lucene-到底长什么样)）。
...
解决方案见 [6.9](#69-深分页的三种方案)：用户翻页用 `search_after`（游标分页），全量遍历用 `PIT + search_after`（ES 7.10+ 的正确做法，`scroll` 已不推荐）。
...
### 考点 4：ES 的写入用了二阶段提交吗？副本写失败怎么办？

> **面试官**：「ES 写入时主分片和副本之间是怎么保证一致的？用的是 2PC 吗？」

**不是 2PC**，是**主备同步复制（Primary-Backup Replication）**。2PC 有 Prepare/Commit 两个阶段、参与者有否决权、失败可全体回滚；而 ES 只有一个阶段——**主分片先自己写成功（既成事实），再并行转发给副本，副本没有否决权，也无法回滚主分片**。

副本写失败时，ES 的处理是**踢掉副本而不是回滚主分片**：主分片上报 Master，把该副本移出 **in-sync 集合**，本次写入仍返回成功，之后 Master 在别处重建副本。只有 in-sync 集合里的副本才有资格被提升为新主分片——这保证了不会把落后的副本提上来导致数据丢失。

**追问「那怎么防止脑裂时的数据错乱」**：靠 `_primary_term`（主分片任期号，每次易主 +1，类比 Raft 的 term）和 `_seq_no`（操作序号）。老主分片"复活"后带着过期的 term 来写，副本直接拒绝。

**追问「并发更新同一文档怎么办」**：ES 没有事务也没有悲观锁，只有基于 `if_seq_no` + `if_primary_term` 的**乐观并发控制**（CAS），冲突返回 409，需应用层重试。详见 [3.2](#32-写入流程)。

### 考点 5：BM25 的三个因子和两个参数
...
**加分项**：主动说明「BM25 调参的收益通常远小于改进分词和加 Rerank（见 [6.11](#611-怎么给-es-加-rerank)），而且改 `similarity` 要重建索引」——体现你知道优化的优先级，而不是死磕参数。

### 考点 6：分片数怎么定？分片是不是越多越好？

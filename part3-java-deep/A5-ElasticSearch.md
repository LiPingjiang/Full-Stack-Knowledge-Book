...
**两个必须知道的术语**——它们**不是 ES 发明的，而是信息检索（IR）领域的通用概念**，Lucene、Solr、甚至学术论文里都是这套叫法：

```
倒排索引 = Term Dictionary（词典） + Postings List（倒排表）

  Term Dictionary（词典）           Postings List（倒排表）
  ┌──────────────┐                ┌────────────────────────┐
  │ "Java"    ───┼───────────────►│ [doc1, doc2]           │
  │ "并发"     ───┼───────────────►│ [doc1, doc3]           │
  │ "编程"     ───┼───────────────►│ [doc1]                 │
  │ "Python"  ───┼───────────────►│ [doc3]                  │
  └──────────────┘                └────────────────────────┘
   所有不同词的集合                  每个词对应的"谁包含我"清单
   （排序存放，便于查找）
```

| 术语 | 中文 | 是什么 |
|------|------|--------|
| **Term** | 词条 | 分词后的最小检索单位，如 `"Java"` |
| **Term Dictionary** | 词典 | 所有 Term 的有序集合 |
| **Postings List** | **倒排表** / 倒排列表 | 一个 Term 对应的**文档号列表**，即"谁包含这个词" |
| **Posting** | 倒排项 | 列表里的单条记录 |

所以 **Postings List 就是上面例子里箭头右边那个 `[doc1, doc2]`**——本书前面说的"文档列表"指的都是它。

实际的 Postings List 比这个例子复杂：除了文档号，还会存**词频**（TF，用于 [BM25 打分](#51-从-tf-idf-到-bm25)）和**词的位置**（用于短语查询 `match_phrase` 和高亮）。它在磁盘上的存储格式、压缩方式和加速结构，详见 [6.11 底层存储引擎](#611-底层存储引擎lucene-到底长什么样)。

> **名字的由来**：`posting` 在英文里有"登记、记账条目"的意思。倒排索引的构建过程就像记账——每处理一篇文档，就在它包含的每个词名下"记一笔"。所有记在同一个词名下的条目汇总起来，就是这个词的 Postings List。
...
ES 写入后**不是立即可搜**，而是有约 1 秒的延迟（`refresh_interval`）。写入流程：数据先进 in-memory buffer → 每秒 refresh 到一个新的 segment（[Lucene](#611-底层存储引擎lucene-到底长什么样) 可搜索的最小单元）→ 后台 merge 合并小 segment。这就是"近实时"的由来。
...
理解 segment 是理解 ES 写入性能的关键。回顾 [3.3](#33-近实时near-real-time原理) 的写入流程，补充完整（Segment 内部的文件结构和 FST 词典见 [6.11](#611-底层存储引擎lucene-到底长什么样)）：
...
关键在于**倒排索引本身也需要索引**。假设有 3000 万个不同的词，怎么快速定位"分布式"这个词的 [Postings List](#21-正排索引-vs-倒排索引)（倒排表，即该词对应的文档号列表）在哪？Lucene 的答案是 **FST（Finite State Transducer，有限状态转换器）**：
...
> 上表是从**使用者视角**看两者的分工。如果想从**存储引擎结构**层面理解这些差异的根源——为什么 ES 无事务、为什么更新代价高、为什么必然是近实时——见 [6.11 底层存储引擎](#611-底层存储引擎lucene-到底长什么样)中 B+ 树与 Lucene Segment 的对比。
...
MySQL FULLTEXT 索引功能有限（中文分词差、不支持复杂查询、不支持分布式）。ES 基于 [Lucene](#611-底层存储引擎lucene-到底长什么样) 的倒排索引，原生支持分词、相关性评分、分布式分片并行查询、丰富的查询 DSL，是专门为搜索场景设计的。

**追问「倒排索引那么大，怎么快速定位到某个词」**：这是区分"背过定义"和"真懂原理"的关键一问。答案是 **FST 词典索引**——倒排索引本身也需要索引，Lucene 把词典压缩成 FST 常驻堆内存，定位后再去磁盘读倒排表（详见 [6.11](#611-底层存储引擎lucene-到底长什么样)）。

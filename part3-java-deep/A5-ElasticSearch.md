...
> **fielddata 是什么，为什么危险**：Doc Values 是**写入时**就建好的磁盘列存；而 `fielddata` 是**查询时**临时把 `text` 字段的整个倒排索引反转成正排结构，并**全量常驻 JVM 堆内存**。一个大文本字段开 fielddata 极易直接打爆堆，所以 ES 默认禁用它，且用熔断器（见 [6.6](#66-集群调优要点)）限制其内存占比。**生产环境的正确做法永远是加 `.keyword` 子字段，而不是开 fielddata。**
...
>
> 具体的恶性循环是这样的：forcemerge 合出一个 50GB 的超大段 → 突破了 `max_merged_segment` 上限 → 此后该段再产生删除文档，常规 merge 也不愿意碰它 → 删除文档越积越多永远清理不掉 → 只能再 forcemerge 一次，代价越来越大。**正确用法是在索引停止写入后（如按天滚动的日志索引的"昨天"那个），于低峰期执行一次**，这也是 ILM Warm 阶段的标配动作（见 [6.5](#65-冷热分离与索引生命周期ilm)）。

...
> **历史案例：MyCat 1.x 的局限性与踩坑经验**
>
> MyCat 作为国内早期分库分表中间件的代表，在 2015-2019 年间被广泛使用。理解其设计局限对选型有警示意义：
>
> **跨分片 JOIN 极弱**：MyCat 1.x 仅支持单分片内的 JOIN 和全局表（每个分片都复制一份的小表）与任意分片的 JOIN。对于跨分片的大表 JOIN，要么强制路由到单一分片执行（失去分片意义），要么由应用层自行拆分查询后组装。这意味着分片后的 SQL 兼容性是断崖式下跌，而非平滑降级。
>
> **XA 事务实现不可靠**：虽然 MyCat 宣称支持 XA 分布式事务，但实际实现存在严重缺陷——二阶段提交中协调者（MyCat 自身）宕机会导致部分参与者锁定资源无法释放。生产环境中几乎需要自行实现业务层补偿或最终一致性方案，中间件的 XA 不能直接使用。
>
> **Schema 变更与平滑迁移困难**：分片规则（如 `rule.xml` 中按 user_id 取模）与业务表强耦合。当需要增加分片数量时（如从 8 片扩到 16 片），必须设计复杂的双写+数据搬迁方案，MyCat 本身不提供在线重分片能力。这在业务快速增长期是致命的运维负担。
>
> **高可用需要自己搭**：MyCat 本身是无状态的，但生产环境需要多节点部署 + 负载均衡（如 LVS/HAProxy）+ 心跳探测。MyCat 节点与 MySQL 节点之间的故障切换没有内建机制，需要配合 MySQL 主从切换工具（如 MHA/ orchestrator）自行串联。这意味着引入 MyCat 后，运维对象从"MySQL 集群"变成了"MyCat 集群 + MySQL 集群 + 负载均衡 + 故障切换链路"，复杂度成倍增加。
>
> **选型启示**：MyCat 的历史经验说明，分库分表中间件的核心价值不仅是"分片路由"，而是**跨分片 SQL 兼容性、在线扩缩容、分布式事务可靠性、高可用自治**这四项能力。ShardingSphere 的崛起很大程度上正是因为它在这些维度上系统性解决了 MyCat 的痛点。当今天评估任何分库分表方案时，都应以上述四项能力作为核心检查清单。

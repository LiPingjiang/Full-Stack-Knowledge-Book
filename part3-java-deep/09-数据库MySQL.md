...
| **C**onsistency（一致性） | 事务前后数据满足约束 | 由 AID 共同保证 |
| **I**solation（隔离性） | 并发事务互不干扰 | 锁 + MVCC |
| **D**urability（持久性） | 提交后数据不会丢 | redo log（重做日志） |

### 3.2 四种隔离级别

| 隔离级别 | 脏读 | 不可重复读 | 幻读 | 性能 |
|---------|------|-----------|------|------|
| Read Uncommitted | 可能 | 可能 | 可能 | 最高（几乎没用） |
| **Read Committed (RC)** | 解决 | 可能 | 可能 | 较高（Oracle 默认） |
| **Repeatable Read (RR)** | 解决 | 解决 | 可能* | 中等（**MySQL 默认**） |
| Serializable | 解决 | 解决 | 解决 | 最低（串行化） |
...
MVCC（Multi-Version Concurrency Control）让读操作不加锁也能看到一致的快照。
...
**产生条件**（四个缺一不可）：互斥、占有且等待、不可抢占、循环等待。

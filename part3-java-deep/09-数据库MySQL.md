...
<details>
<summary><b>展开：索引的两个分类维度——不是一回事</b></summary>

索引有两个独立的分类维度，不要混淆：

| 维度 | 分类 | 区分依据 |
|------|------|---------|
| **存储方式** | 聚簇索引 vs 二级索引（非聚簇） | 叶子节点存整行数据还是只存主键 |
| **列数** | 单列索引 vs 联合索引 | 索引建在一个列上还是多个列上 |

两个维度可以自由组合——联合索引绝大多数情况下是二级索引，比如 `INDEX idx_name_age (name, age)` 就是一个联合的二级索引。理论上聚簇索引也可以是"联合"的：如果主键是 `PRIMARY KEY (order_id, item_id)`，那聚簇索引就按这两列排序，叶子节点存完整行。只不过联合主键在实践中较少见，所以提到"联合索引"时默认指二级索引。

**聚簇 vs 二级是 InnoDB 的固定规则，不是你选的**：每张表有且只有一棵聚簇索引，就是主键。你建表时定义了 `PRIMARY KEY (id)`，InnoDB 自动把数据行按 id 排序存储在聚簇索引的叶子节点上。你额外创建的所有索引（不管单列还是联合）全部都是二级索引。查询时优化器选择的是"走哪棵索引树"，而不是"用聚簇还是非聚簇模式"——因为聚簇索引只有一棵，数据物理上只存一份。

**B+ 树的排序由 collation 决定**：创建索引时 MySQL 自动按索引列的值排序构建 B+ 树，排序规则由建表时指定的 collation（排序规则）控制，比如 `utf8mb4_general_ci` 按 Unicode 通用规则排，`utf8mb4_0900_ai_ci`（MySQL 8.0 默认）按 Unicode 9.0 标准排。上面二级索引示意图中，6 个名字按拼音排序为：李四(L) → 孙七(S) → 王五(W) → 张三(Z) → 赵六(Z) → 周八(Z)，分界值 `[张]` 把它们分成两组，这个排序和分组都是 InnoDB 自动维护的——插入、删除数据时自动调整树结构保持有序。
...
redo log 和 binlog 是两个独立的日志系统（一个在引擎层，一个在 Server 层），如何保证它们的一致性？用**两阶段提交（2PC）**——先让 redo log 落盘并标记 state=PREPARE，再写 binlog 落盘，最后把 redo log 标记为 COMMIT。这样 crash 后恢复程序可以根据两份日志的状态裁决每个事务是提交还是回滚。

一个事务从执行到提交分为三个阶段：**改内存（全在内存）→ 落日志（两阶段提交，保证日志写入完整）→ 异步落数据（后台线程刷脏页，与前两阶段完全解耦）**。阶段一和阶段二严格串行；阶段三随时可能发生（Page Cleaner 不关心事务阶段），这也是为什么 crash 后即使事务未提交也需要用 undo log 回滚磁盘数据页。

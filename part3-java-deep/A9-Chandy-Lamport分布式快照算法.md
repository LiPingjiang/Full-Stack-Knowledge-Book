...
- [二、关键概念澄清](#二关键概念澄清)
  ...
  - [2.4 Marker(id=N) 消息](#24-markeridn-消息)
...
> **说明**：d4-d6 / e4-e8 都是 Barrier(i) 之前的数据，属于 Checkpoint(i) 的输入范围。
> 输入流 a 的 Barrier(i) 比输入流 b 早到达算子。
...
```mermaid
sequenceDiagram
    participant a as 输入流 a
    participant b as 输入流 b
    participant op as 算子
    participant ck as Checkpoint 存储
    ...
    a->>op: ★ Barrier(i) 到达
    Note over op: 暂停输入流 a<br/>后续数据进缓冲区，不处理
    Note over op: 继续处理输入流 b

    a-->>op: d7 → 缓冲区（暂不处理）
    b->>op: 处理 e7
    b->>op: 处理 e8

    b->>op: ★ Barrier(i) 到达，对齐完成
    Note over op: 所有输入流 Barrier(i) 已到<br/>d4~d6 + e4~e8 全部处理完<br/>结果已在算子状态中

    op->>ck: 保存算子状态（含 d4~d6 + e4~e8 的处理结果）
    op->>ck: 信道状态 a = 空
    op->>ck: 信道状态 b = 空
    Note over ck: ✅ Checkpoint(i) 完成，上报 JobManager

    Note over op: 从缓冲区取出 d7，继续处理
    op->>op: 处理 d7（属于 Checkpoint(i+1) 范围）
    b->>op: 处理 e9（属于 Checkpoint(i+1) 范围）
```
...
**代价**：等待对齐期间，d7 积压在缓冲区不处理。反压严重时，b 的 Barrier(i) 迟迟不来，d7 等待时间很长，拖慢整体吞吐。
...
sequenceDiagram
    ...
    a->>op: ★ Barrier(i) 到达，立即触发
    Note over op: 不暂停任何输入流<br/>立即开始保存快照

    op->>ck: ① 保存算子状态（含 d4~d6 + e4~e6 的处理结果）
    op->>ck: ② 信道状态 a 初始 = 空（a 的 Barrier 已到，之后的 d7 存入）

    Note over op: 算子继续正常处理，同时收集信道状态 b

    a->>op: d7 到达 → 正常处理（属于 Checkpoint(i+1)，不存入本次信道状态）
    b->>op: e7 到达 → 存入信道状态 b（Barrier(i) 之前的在途数据）
    b->>op: e8 到达 → 存入信道状态 b（Barrier(i) 之前的在途数据）

    op->>ck: ③ 信道状态 b 持续追加：[e7, e8]

    b->>op: ★ Barrier(i) 到达，信道状态 b 封存
    op->>ck: 信道状态 b 最终 = [e7, e8]
    Note over ck: ✅ Checkpoint(i) 完成，上报 JobManager
...
| 组成 | 内容 | 说明 |
|------|------|------|
| 算子状态 | 已处理 d4/d5/d6 + e4/e5/e6 后的状态 | 触发时刻的快照 |
| 信道状态 a | **空** | a 的 Barrier(i) 已到，之前数据均已处理 |
| 信道状态 b | **[e7, e8]** | b 的 Barrier(i) 未到，这两条是在途数据 |

> **d4~d6 / e4~e6 为什么不存入快照？**
> 它们在 Barrier(i) 到达之前就已被算子正常处理，结果已反映在算子状态里。存入快照的只是**还没被处理的在途数据**（信道状态）。

> **d7 为什么不存入信道状态 b？**
> d7 是 a 的 Barrier(i) **之后**的数据，属于 Checkpoint(i+1) 的输入范围，不归本次快照管。

**从 Checkpoint(i) 恢复时**：恢复算子状态 → 把 e7/e8 重新注入输入流 b → 算子重新处理 → 语义正确，不丢不重。
...
```
时间 →
输入流 a：[d4][d5][d6][Barrier(i)][d7]...
输入流 b：[e4][e5][e6][e7][e8][Barrier(i)][e9]...

对齐模式：
  t1: a 的 Barrier(i) 到 → 暂停 a，d7 进缓冲区
  t2: 继续处理 b：e7、e8
  t3: b 的 Barrier(i) 到 → 对齐完成 → 保存状态（空信道状态）→ 完成
  t4: 从缓冲区取出 d7 处理

  多处理了：e7、e8（对齐等待期间继续处理 b）
  多缓存了：d7（等待期间 a 的数据积压）

非对齐模式：
  t1: a 的 Barrier(i) 到 → 立即触发，保存算子状态 + 信道状态 a=空
  t2: 继续处理 a：d7（正常处理，不存入本次快照）
  t2: 继续处理 b：e7→存信道状态b，e8→存信道状态b
  t3: b 的 Barrier(i) 到 → 信道状态 b=[e7,e8] 封存 → 完成

  多处理了：d7（对齐等待期间 a 继续处理）
  多缓存了：e7、e8（存入信道状态，恢复时重新注入）
```
...
Flink 里的"信道"就是**算子之间的网络传输缓冲区**，物理上对应：

```
上游算子 → [NetworkBuffer / InputChannel] → 下游算子
```

每个输入流对应一个 InputChannel，内部有一个缓冲队列，数据按 FIFO 顺序排队等待算子消费。Barrier(i) 就混在这个队列里，和普通数据一起按顺序流动。

#### 信道状态存的是原始数据，不是处理结果

一个容易产生的疑问：e7/e8 已经被算子处理了，为什么不直接把处理结果写入 Checkpoint(i)，而是要存原始数据？

原因在于**算子状态快照是在触发瞬间一次性拍的**：

```
非对齐触发时刻（a 的 Barrier(i) 到达）：
  算子状态快照 = 此刻内存状态（含 d4~d6 + e4~e6 的处理结果）
  ↑ 快照拍完后就固定了，之后算子继续跑，快照不再更新

触发之后，e7/e8 陆续到来并被处理：
  → 算子内存状态继续变化（含了 e7/e8 的结果）
  → 但这些变化没有反映在已拍好的算子状态快照里
  → 一旦故障，e7/e8 的处理结果就丢了

解决方案：把 e7/e8 的原始数据存入信道状态
  → 恢复时：先恢复算子状态快照（不含 e7/e8）
  → 再重放信道状态里的 e7/e8
  → 算子重新处理，结果还原，语义正确
```
...
理论上存处理结果（增量状态变化）也能恢复，但实现复杂度极高：

- 算子状态是一个整体（如 RocksDB），不是一条条记录
- e7 处理后可能修改了状态里的多个 key，很难精确追踪"e7 导致了哪些状态变化"
- 存原始数据 + 恢复时重放，实现简单、语义清晰

| 方案 | 可行性 | 实现复杂度 | Flink 选择 |
|------|--------|-----------|-----------|
| 存原始数据，恢复时重放 | ✅ | 低 | ✅ 采用 |
| 存增量状态变化 | ✅ | 高（需追踪每条数据影响的 key） | ❌ 未采用 |

> **一句话**：信道状态存的是**原始数据**，不是处理结果。算子状态快照在触发瞬间已拍完，之后处理的数据结果不在快照里，必须存原始数据以便恢复时重放。

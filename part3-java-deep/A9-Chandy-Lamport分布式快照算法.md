...
> **在途消息** = 在 Pi 的快照时刻已经发出（反映在 Si 里），但在 Pj 的快照时刻还没被 Pj 处理（不在 Sj 里）的消息。

这些消息物理上**会到达 Pj**，Pj 收到后把它们**存档为信道状态**，而不是直接处理。恢复时再重新消费，保证不丢不重。
...
如果只记录进程状态，忽略信道状态，快照就不完整：

```
P2 发出 $50 转账（m 在途）
P2 快照 S2：余额 $150（已扣款，m 已发出）
P3 快照 S3：余额 $300（m 还没到 Pj 的快照时刻）
→ 如果不记录信道状态，$50 凭空消失！

正确做法：
S2 = $150，S3 = $300，信道 C23 状态 = [$50]
→ 总量守恒：$150 + $300 + $50 = $600 ✅
恢复时 P3 重新消费 $50，最终 P2=$150，P3=$350
```

**Pj 如何知道哪些消息属于信道状态？靠 FIFO + Marker 位置：**

```
信道 Cij 上消息到达 Pj 的顺序（FIFO 保证顺序）：

  [m1] [m2] [Marker(id=N)] [m3] [m4] ...
   ↑    ↑         ↑          ↑    ↑
  正常  正常    快照边界    存入   下一个
  处理  处理               信道   快照的
  进Sj  进Sj               状态   消息
...
- Marker 之前到达的 m1、m2：Pj 正常处理，结果已反映在 Sj 里
- Marker 之后、下一个 Marker 之前到达的 m3：Pj 收到后**存入信道状态**（不更新业务状态）
- 收到 Marker 后停止记录，信道状态封存

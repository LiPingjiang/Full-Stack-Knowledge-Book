...
```
P2（发送方）发出 $50 转账消息 m，P3（接收方）尚未处理
P2 快照 S2：余额 $150（已扣款，m 已发出，反映在 S2 里）
P3 快照 S3：余额 $300（m 还没到 P3 的快照时刻，S3 里没有 m）
→ 如果不记录信道状态，$50 凭空消失！

正确做法：
S2 = $150，S3 = $300，信道 C23（P2→P3）状态 = [$50]
→ 总量守恒：$150 + $300 + $50 = $600 ✅
恢复时 P3（接收方）重新消费 $50，最终 P2=$150，P3=$350
```

**Pj（接收方）如何知道哪些消息属于信道状态？靠 FIFO + Marker 位置：**

```
信道 Cij（Pi→Pj）上消息到达 Pj（接收方）的顺序（FIFO 保证顺序）：
...
- Marker 之前到达的 m1、m2：Pj（接收方）正常处理，结果已反映在 Sj 里
- Marker 之后到达的 m3：Pj（接收方）收到后**存入信道状态**（不更新业务状态）

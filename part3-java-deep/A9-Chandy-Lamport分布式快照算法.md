...
sequenceDiagram
    ...
    Note over Pi: 快照前正常运行
    Pi->>Pj: m1（普通消息）
    Pi->>Pj: m2（普通消息）

    Note over Pj: Pj 收到 m1、m2，正常处理<br/>结果已反映在 Sj 里（非在途）

    rect rgb(255, 230, 230)
        Note over Pi: ★ Pi 的快照时刻<br/>记录状态 Si<br/>（m1、m2 已发出，体现在 Si 里）
        Pi->>Pj: m3（普通消息，Pi 快照后仍在传输中）
        Note over Pi: 向 Pj 发送 Marker(id=N)
        Pi->>Pj: Marker(id=N)
    end

    rect rgb(230, 245, 255)
        Note over Pj: ★ Pj 的快照时刻<br/>记录状态 Sj<br/>（m3 尚未到达，Sj 里没有 m3）
        Pj->>Pj: 收到 m3 → 存入信道状态（不处理）<br/>⚠️ 在途消息：Pi 已记录发出，Pj 未记录处理
        Pj->>Pj: 收到 Marker(id=N) → 信道状态封存<br/>Cij 状态 = [m3]
    end

    Pi->>Pj: m4（Marker 之后发出，属于下一个快照）
    Note over Pj: m4 正常处理，不属于本次快照
```

**三类消息对比总结：**

| 消息 | Pi 发出时刻 | Pj 处理时刻 | 归属 |
|------|------------|------------|------|
...
```
P2（发送方）发出 $50 转账消息 m3，P3（接收方）尚未处理
P2 快照 S2：余额 $150（已扣款，m3 已发出，反映在 S2 里）
P3 快照 S3：余额 $300（m3 还没到 P3 的快照时刻，S3 里没有 m3）
→ 如果不记录信道状态，$50 凭空消失！
...
- m1、m2（Marker 之前到达）：Pj 正常处理，结果已反映在 Sj 里
- m3（Marker 之后到达）：Pj 收到后**存入信道状态**，不更新业务状态
- m4（下一个 Marker 之后）：属于下一个快照，不属于本次信道状态
